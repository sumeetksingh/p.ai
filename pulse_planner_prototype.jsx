    ...
    <div>
      ...
      <section className="mt-4 rounded-2xl border border-gray-200 bg-white p-5">
        ...
        <div className="mt-5 flex items-stretch gap-3 overflow-x-auto pb-2">
          ...
          {waves.map((item,index) => { const tone = pulseScoreTone(item.after); const active = safeSelectedWave === index; return <button key={item.number} onClick={() => setSelectedWave(index)} className={`min-w-[150px] rounded-xl border p-3 text-left transition ${active ? 'border-blue-600 ring-2 ring-blue-100 bg-blue-50/40' : 'border-gray-200 hover:border-blue-300'}`}><div className="flex justify-between items-start"><span className="text-[10px] text-gray-500">Wave {item.number} · {item.month.split(' ')[0]}</span>{index === 0 && launched && <span className="text-[9px] text-blue-700">LOCKED</span>}</div><div className="mt-1 flex items-center gap-1.5"><strong className="text-lg">{item.before}</strong><ArrowRight size={12} className="text-gray-400" /><strong className={`text-lg ${tone.text}`}>{item.after}</strong></div><div className="mt-2 h-1.5 rounded-full bg-gray-100 overflow-hidden"><div className={`h-full rounded-full ${tone.bg}`} style={{ width: `${item.after}%` }} /></div><div className="mt-2 text-[10px] text-gray-500">${item.returnBudget.toLocaleString()} returns · +{item.newlyAligned.toLocaleString()} aligned</div></button>; })}
        </div>
      </section>

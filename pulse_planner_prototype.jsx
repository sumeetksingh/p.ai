  ...
  return (
    <aside className="w-56 bg-white border-r border-gray-200 px-3 py-4 flex flex-col flex-shrink-0">
      ...
      <nav className="flex-1 space-y-0.5">
        ...
        <NavBtn id="get-well" icon={Power} label="Get well enablement" active={current === 'get-well'} accent badge="5 ON" />
        <div className="my-2 border-t border-gray-200"></div>
      ...
      {view !== 'plan' ? (
        ...
        <div className="grid min-h-[700px]" style={{ gridTemplateColumns: '235px minmax(560px,1fr) 310px' }}>
          <aside className="border-r border-[#ded8ca] p-5 bg-[#faf7ef]/60">
            ...
            <div className="mt-6 pt-5 border-t border-[#ded8ca]"><div className="flex justify-between items-center"><div><div className="text-[10px] uppercase tracking-[0.14em] font-semibold text-[#8d8678]">Digital worker</div><div className="text-sm font-semibold mt-1">Get well worker</div></div><button onClick={() => setWorkerOn(!workerOn)} className={`w-11 h-6 rounded-full p-0.5 transition-colors ${workerOn ? 'bg-[#1f1d19]' : 'bg-[#d4cec1]'}`}><span className={`block w-5 h-5 rounded-full bg-white transition-transform ${workerOn ? 'translate-x-5' : ''}`} /></button></div><div className={`mt-3 rounded-xl border px-3 py-2.5 text-[11px] ${workerOn ? 'border-[#cbd9c2] bg-[#edf3e8] text-[#3f6335]' : 'border-[#ddd6c9] bg-white/40 text-[#817a6f]'}`}>{workerOn ? 'On · Watching daily rankings' : 'Paused · Plan will not refresh'}</div></div>
          ...
          <main className="p-6">
            <div className="flex items-start justify-between"><div><h2 className="text-xl font-semibold tracking-[-0.025em]">Get well journey</h2><p className="text-xs text-[#7d776c] mt-1">Each folder is one monthly replacement wave. Open a wave to review its work.</p></div><div className="inline-flex items-center gap-2 rounded-full border border-[#d7d0c0] bg-white/55 px-3 py-1.5 text-[11px]"><Sparkles size={13} className="text-[#a37722]" />Refreshed today at 6:00 AM</div></div>
...
function GetWellEnablementScreen({ onNavigate }) {
  ...
  return (
    <div className="diskbuddy-surface -m-6 min-h-screen bg-[#f7f3e8] text-[#201f1c]">
      <div className="h-14 border-b border-[#ded8ca] px-6 flex items-center justify-between bg-[#faf7ef]"><div className="flex items-center gap-2"><span className="w-8 h-8 rounded-lg bg-[#1f1d19] text-white flex items-center justify-center"><Power size={15} /></span><strong className="text-sm">Get well enablement</strong></div><label className="relative"><Search size={14} className="absolute left-3 top-2.5 text-[#8b857a]" /><input className="w-64 rounded-full border border-[#d8d1c2] bg-white/60 py-2 pl-9 pr-3 text-xs outline-none" placeholder="Find a store..." /></label></div>
      <div className="px-6 py-5 border-b border-[#ded8ca]"><div className="flex justify-between items-end"><div><h1 className="text-3xl font-semibold tracking-[-0.035em]">Digital workers</h1><p className="text-xs text-[#7e786d] mt-1">Enable a store and Pulse will build, refresh, and monitor its capacity-aware Get Well Plan.</p></div><button onClick={() => setEnabled(new Set(ENABLEMENT_STORES.map(store => store.id)))} className="rounded-full bg-[#1f1d19] text-white px-4 py-2.5 text-xs font-medium flex items-center gap-2"><Sparkles size={14} />Enable all ready stores</button></div></div>
...
function PulseStoreWorkspaceScreen({ onNavigate }) {
  ...
  if (view !== 'plan') {
    return (
      <div>
        <Breadcrumb items={['Assortment overview', 'Store health', 'ATL-050']} />
        <div className="flex items-end justify-between"><div><h1 className="text-[22px] font-medium">ATL-050 (DOWNTOWN ATLANTA)</h1><p className="text-sm text-gray-500 mt-1">Assortment overview</p></div><div className="inline-flex rounded-full border border-gray-200 bg-white p-1">{[['overview','Overview'],['categories','Category health'],['plan','Get well plan']].map(([id,label]) => <button key={id} onClick={() => setView(id)} className={`rounded-full px-4 py-2 text-xs ${view === id ? 'bg-gray-950 text-white' : 'text-gray-600'}`}>{label}</button>)}</div></div>
        <div className="mt-5 grid grid-cols-3 gap-4"><section className="rounded-2xl border border-gray-200 bg-white p-5"><div className="text-xs text-gray-500">Assortment health score</div><strong className="text-5xl block mt-4">29</strong><span className="text-xs text-red-700">Critical · 71% not aligned</span></section><section className="col-span-2 rounded-2xl border border-gray-200 bg-white p-5"><h2 className="font-medium">{view === 'overview' ? 'Assortment snapshot' : 'Category health'}</h2><div className="mt-5 space-y-3">{STORE_CATEGORIES.map(row => <div key={row.name} className="grid grid-cols-[180px_1fr_45px] items-center gap-3 text-xs"><span>{row.name}</span><div className="h-2 rounded-full bg-gray-100 overflow-hidden"><div className="h-full bg-red-600" style={{ width: `${Math.max(5,row.score)}%` }} /></div><strong>{row.score}</strong></div>)}</div></section></div>
      </div>
    );
  }
  ...
  return (
    <div>
      <Breadcrumb items={['Assortment overview', 'Store health', 'ATL-050', 'Get well plan']} />
      <div className="flex items-start justify-between gap-4">
      ...
      <div className="mt-4 flex items-center justify-between"><div className="inline-flex rounded-full border border-gray-200 bg-white p-1">{[['overview','Overview'],['categories','Category health'],['plan','Get well plan']].map(([id,label]) => <button key={id} onClick={() => setView(id)} className={`rounded-full px-4 py-2 text-xs ${view === id ? 'bg-gray-950 text-white' : 'text-gray-600 hover:bg-gray-50'}`}>{label}</button>)}</div><div className="flex items-center gap-2 text-xs text-gray-500"><RefreshCw size={13} />Recommendations refreshed today</div></div>
      ...
      <section className="mt-4 rounded-2xl border border-gray-200 bg-white p-5">
        <div className="flex items-start justify-between"><div><h2 className="font-medium">Get well journey</h2><p className="text-xs text-gray-500 mt-1">Select a wave to see which category and SKU decisions create its health-score lift.</p></div><HealthLegend /></div>
...
function PulseGetWellEnablementScreen({ onNavigate }) {
  ...
  return (
    <div>
      <Breadcrumb items={['Pulse AI', 'Get well enablement']} />
      <div className="flex items-start justify-between"><div><h1 className="text-[22px] font-medium">Get well enablement</h1><p className="text-sm text-gray-500 mt-1">Turn on a digital worker after the DC assigns monthly capacity. Review and edit each journey inside its store page.</p></div><button onClick={() => setEnabled(new Set(ENABLEMENT_STORES.map(store => store.id)))} className="rounded-full bg-blue-700 hover:bg-blue-800 text-white px-4 py-2.5 text-xs font-medium flex items-center gap-2"><Sparkles size={14} />Enable all ready stores</button></div>

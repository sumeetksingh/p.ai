  ...
  return (
    <div>
      ...
      <div className="mt-3 rounded-2xl border border-gray-200 bg-white px-5 py-4 flex items-center gap-4 shadow-sm">
        ...
        <div className="flex-1"><div className="text-sm font-medium">Move from recommendations to execution</div><p className="text-xs text-gray-500 mt-1">Sequence the worst-ranked Returns within the DC’s monthly dollar budget, then show the best missing products to Order alongside them.</p></div>
...
const ENABLEMENT_STORES = [
  { id: 'ATL-050', name: 'DOWNTOWN ATLANTA', score: 29, target: 91, actions: 148600, capacity: 10000, waves: 15, status: 'Ready for review' },
  { id: 'ATL-466', name: 'FORT GAINES GA', score: 31, target: 88, actions: 68400, capacity: 12000, waves: 6, status: 'Wave 1 active' },
  { id: 'ATL-328', name: 'WASHINGTON GA', score: 28, target: 91, actions: 72000, capacity: 15000, waves: 5, status: 'Plan refreshed' },
  { id: 'ATL-191', name: 'EATONTON, GA', score: 31, target: 90, actions: 113000, capacity: 20000, waves: 6, status: 'Needs review' },
  { id: 'ATL-130', name: 'GSE IBS', score: 33, target: 94, actions: 9600, capacity: 5000, waves: 2, status: 'Ready to launch' },
  { id: 'ATL-330', name: 'BEASLEY AUTO PARTS', score: 34, target: 89, actions: 82200, capacity: 15000, waves: 6, status: 'Not enabled' },
  { id: 'ATL-440', name: 'ARLINGTON GA', score: 34, target: 92, actions: 74400, capacity: 12500, waves: 6, status: 'Not enabled' },
  { id: 'ATL-229', name: 'MARIETTA EAST', score: 38, target: 93, actions: 51800, capacity: 10000, waves: 6, status: 'Not enabled' },
];
...
const RETURN_VALUE_BACKLOG = 148600;

function scoreBand(score) {
  return score <= 50 ? 'Critical' : score <= 70 ? 'At risk' : 'Excellent';
}

function buildReturnBudgetJourney(monthlyBudget) {
  const safeBudget = Math.max(1000, Number(monthlyBudget) || 10000);
  const waveCount = Math.min(24, Math.ceil(RETURN_VALUE_BACKLOG / safeBudget));
  const categoryModels = [
    { name: 'Brakes', start: 18, target: 88 },
    { name: 'Body & Accessories', start: 30, target: 84 },
    { name: 'Steering & Suspension', start: 32, target: 82 },
    { name: 'Tools & Equipment', start: 35, target: 79 },
  ];
  return Array.from({ length: waveCount }, (_, index) => {
    const spentBefore = Math.min(RETURN_VALUE_BACKLOG, index * safeBudget);
    const returnBudget = Math.min(safeBudget, RETURN_VALUE_BACKLOG - spentBefore);
    const spentAfter = spentBefore + returnBudget;
    const progressBefore = spentBefore / RETURN_VALUE_BACKLOG;
    const progressAfter = spentAfter / RETURN_VALUE_BACKLOG;
    const before = Math.round(29 + 62 * Math.pow(progressBefore, .74));
    const after = Math.round(29 + 62 * Math.pow(progressAfter, .74));
    const returnUnits = Math.max(1, Math.round(returnBudget / (16.4 + index * .7)));
    const orderUnits = Math.round(returnUnits * (1.32 - Math.min(.18, index * .012)));
    const alignedBefore = Math.round(12000 + 17200 * Math.pow(progressBefore, .82));
    const aligned = Math.round(12000 + 17200 * Math.pow(progressAfter, .82));
    const date = new Date(2026, 8 + index, 1);
    const categories = categoryModels.map((category, categoryIndex) => {
      const categoryBefore = Math.round(category.start + (category.target - category.start) * Math.pow(progressBefore, .72));
      const categoryAfter = Math.round(category.start + (category.target - category.start) * Math.pow(progressAfter, .72));
      return { name: category.name, before: categoryBefore, after: categoryAfter, from: scoreBand(categoryBefore), to: scoreBand(categoryAfter), aligned: Math.max(12, Math.round((aligned - alignedBefore) * [0.31,0.27,0.23,0.19][categoryIndex])) };
    });
    return {
      number: index + 1,
      month: date.toLocaleDateString('en-US', { month: 'long', year: 'numeric' }),
      before, after, aligned, newlyAligned: aligned - alignedBefore,
      remaining: Math.max(0, 42000 - aligned),
      remainingReturnValue: Math.max(0, RETURN_VALUE_BACKLOG - spentAfter),
      returnBudget, returnUnits, orderUnits,
      status: index === 0 ? 'Ready for review' : 'Refreshes before launch',
      strategySkus: Math.max(74, Math.round(842 * (1 - progressBefore * .72))),
      categories,
    };
  });
}
...
function PulseStoreWorkspaceScreenLegacy({ onNavigate }) {
  const [view, setView] = useState('overview');
  const [selectedWave, setSelectedWave] = useState(0);
  const [launched, setLaunched] = useState(false);
  const [budget, setBudget] = useState(10000);
  const [strategyEditorOpen, setStrategyEditorOpen] = useState(false);
  const [scheduledStrategies, setScheduledStrategies] = useState(() => ({
    8: {
      title: 'Expand Spark Plugs assortment',
      detail: 'Bring in more top-ranked Spark Plugs before this wave is built.',
      status: 'Scheduled',
    },
  }));
  const waves = useMemo(() => buildReturnBudgetJourney(budget), [budget]);
  const safeSelectedWave = Math.min(selectedWave, waves.length - 1);
  const wave = waves[safeSelectedWave];
  const scheduledStrategy = scheduledStrategies[safeSelectedWave];
  const waveLocked = safeSelectedWave === 0 && launched;
  ...
  const scheduleStrategy = () => {
    ...
    setScheduledStrategies(previous => ({
      ...previous,
      [safeSelectedWave]: {
        title: 'Expand Spark Plugs assortment',
        detail: 'Bring in more top-ranked Spark Plugs before this wave is built.',
  ...
  return (
    <div>
      ...
      <section className="mt-4 rounded-2xl bg-blue-950 text-white px-5 py-4 flex items-center gap-4"><div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center"><Beaker size={18} /></div><div className="flex-1"><div className="text-xs text-blue-200">Scenario Planner strategy applied</div><div className="text-sm font-medium mt-0.5">ATL-050 Q4 assortment strategy · 4 active rules</div><p className="text-[11px] text-blue-200 mt-1">For Brake Pads, Brembo is deprioritized. Pulse uses the strategy to re-rank future Orders and Returns.</p></div><button onClick={() => onNavigate('planner-door')} className="rounded-lg border border-white/20 px-3.5 py-2 text-xs flex items-center gap-2">Review strategy <ArrowRight size={13} /></button></section>

      <div className="grid grid-cols-4 gap-3 mt-4">
        <div className="rounded-2xl border border-gray-200 bg-white p-4"><span className="text-xs text-gray-500">Health journey</span><div className="mt-2 flex items-center gap-2"><strong className="text-2xl">29</strong><ArrowRight size={15} className="text-gray-400" /><strong className="text-2xl text-green-700">91</strong></div><small className="text-[11px] text-gray-500">Critical → Excellent</small></div>
        <div className="rounded-2xl border border-gray-200 bg-white p-4"><span className="text-xs text-gray-500">Return-value backlog</span><strong className="text-2xl block mt-2">$148,600</strong><small className="text-[11px] text-gray-500">Recommended inventory value</small></div>
        <div className="rounded-2xl border border-gray-200 bg-white p-4"><span className="text-xs text-gray-500">Monthly return budget</span><div className="mt-1 flex items-baseline gap-1"><span className="text-xl font-semibold">$</span><input aria-label="Monthly return budget" type="number" min="1000" step="1000" value={budget} onChange={event => setBudget(event.target.value)} className="w-24 text-2xl font-semibold outline-none border-b border-dashed border-gray-300" /></div><small className="text-[11px] text-gray-500">Maximum return value accepted by DC</small></div>
        <div className="rounded-2xl border border-gray-200 bg-white p-4"><span className="text-xs text-gray-500">Estimated recovery</span><strong className="text-2xl block mt-2">{waves.length} waves</strong><small className="text-[11px] text-gray-500">Complete {waves[waves.length-1].month}</small></div>
      </div>
      ...
      <section className="mt-4 rounded-2xl border border-gray-200 bg-white p-5">
        ...
        <div className="mt-5 flex items-stretch gap-3 overflow-x-auto pb-2">
          <div className="rounded-xl border border-gray-200 bg-gray-50 p-3 min-w-[110px]"><div className="text-[10px] text-gray-500">Today</div><strong className="text-xl">29</strong><div className="mt-2 h-1.5 rounded-full bg-gray-200"><div className="h-full w-[29%] rounded-full bg-red-600" /></div></div>
          {waves.map((item,index) => { const tone = pulseScoreTone(item.after); const active = safeSelectedWave === index; const strategyChange = scheduledStrategies[index]; return <button key={item.number} onClick={() => { setSelectedWave(index); setStrategyEditorOpen(false); }} className={`min-w-[160px] rounded-xl border p-3 text-left transition ${active ? 'border-blue-600 ring-2 ring-blue-100 bg-blue-50/40' : 'border-gray-200 hover:border-blue-300'}`}><div className="flex justify-between items-start"><span className="text-[10px] text-gray-500">Wave {item.number} · {item.month.split(' ')[0]}</span>{index === 0 && launched && <span className="text-[9px] text-blue-700">LOCKED</span>}</div><div className="mt-1 flex items-center gap-1.5"><strong className="text-lg">{item.before}</strong><ArrowRight size={12} className="text-gray-400" /><strong className={`text-lg ${tone.text}`}>{item.after}</strong></div><div className="mt-2 h-1.5 rounded-full bg-gray-100 overflow-hidden"><div className={`h-full rounded-full ${tone.bg}`} style={{ width: `${item.after}%` }} /></div><div className="mt-2 text-[10px] text-gray-500">${item.returnBudget.toLocaleString()} returns · +{item.newlyAligned.toLocaleString()} aligned</div>{strategyChange && <div className="mt-2 rounded-md bg-blue-50 border border-blue-100 px-2 py-1 text-[9px] font-medium text-blue-800 flex items-center gap-1"><CalendarDays size={10} />Strategy change</div>}</button>; })}
        </div>
      </section>

      <div className="mt-4 grid grid-cols-[1.35fr_.65fr] gap-4">
        <section className="rounded-2xl border border-gray-200 bg-white overflow-hidden"><div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between"><div><h2 className="font-medium">Wave {wave.number} category impact</h2><p className="text-xs text-gray-500 mt-1">{transitionCount} categories cross a health threshold this month.</p></div><span className="rounded-full bg-green-50 border border-green-200 text-green-800 px-3 py-1 text-[11px]">+{wave.newlyAligned.toLocaleString()} aligned</span></div><div className="divide-y divide-gray-100">{wave.categories.map(category => { const afterTone = pulseScoreTone(category.after); return <div key={category.name} className="px-5 py-3 grid grid-cols-[180px_1fr_150px] items-center gap-4"><div><strong className="text-xs block">{category.name}</strong><small className="text-[10px] text-gray-500">+{category.aligned.toLocaleString()} aligned</small></div><div><div className="flex justify-between text-[10px] text-gray-500"><span>{category.before}</span><span className={afterTone.text}>{category.after}</span></div><div className="relative mt-1.5 h-2 rounded-full bg-gray-100 overflow-hidden"><div className="absolute inset-y-0 left-0 bg-gray-300 rounded-full" style={{ width: `${category.before}%` }} /><div className={`${afterTone.bg} absolute inset-y-0 left-0 rounded-full opacity-90`} style={{ width: `${category.after}%` }} /></div></div><div className="flex items-center justify-end gap-1.5 text-[10px]"><span className="rounded-full border border-gray-200 px-2 py-1 text-gray-600">{category.from}</span><ArrowRight size={11} className="text-gray-400" /><span className={`rounded-full border px-2 py-1 ${afterTone.soft} ${afterTone.text}`}>{category.to}</span></div></div>; })}</div></section>
        <aside className="rounded-2xl border border-gray-200 bg-white p-5"><div className="flex items-center justify-between"><div><div className="text-[10px] uppercase tracking-wider text-gray-500">Selected wave</div><h2 className="font-medium mt-1">Wave {wave.number} · {wave.month}</h2></div><span className="rounded-full bg-blue-50 text-blue-700 border border-blue-200 px-2.5 py-1 text-[10px]">{waveLocked ? 'Locked' : wave.status}</span></div><div className="mt-5 rounded-xl bg-gray-50 p-4"><div className="text-xs text-gray-500">Health score impact</div><div className="mt-2 flex items-center gap-2"><strong className="text-3xl">{wave.before}</strong><ArrowRight size={16} className="text-gray-400" /><strong className="text-3xl text-green-700">{wave.after}</strong></div></div><dl className="mt-4 space-y-3 text-xs"><div className="flex justify-between"><dt className="text-gray-500">Return budget used</dt><dd className="font-semibold">${wave.returnBudget.toLocaleString()}</dd></div><div className="flex justify-between"><dt className="text-gray-500">Return SKUs selected</dt><dd className="font-semibold">{wave.returnUnits.toLocaleString()}</dd></div><div className="flex justify-between"><dt className="text-gray-500">Order SKUs recommended</dt><dd className="font-semibold">{wave.orderUnits.toLocaleString()}</dd></div><div className="flex justify-between"><dt className="text-gray-500">Return value remaining</dt><dd className="font-semibold">${wave.remainingReturnValue.toLocaleString()}</dd></div><div className="flex justify-between"><dt className="text-gray-500">SKUs affected by strategy</dt><dd className="font-semibold">{wave.strategySkus.toLocaleString()}</dd></div></dl><div className={`mt-4 rounded-xl border p-3 ${scheduledStrategy ? 'border-blue-200 bg-blue-50' : 'border-gray-200 bg-white'}`}><div className="flex items-center gap-2 text-xs font-medium"><CalendarDays size={13} className={scheduledStrategy ? 'text-blue-700' : 'text-gray-500'} />Strategy before this wave</div>{scheduledStrategy ? <><strong className="text-xs block mt-2 text-blue-950">{scheduledStrategy.title}</strong><p className="text-[10px] text-blue-800 mt-1">{scheduledStrategy.detail}</p><span className="inline-block mt-2 rounded-full bg-white border border-blue-200 px-2 py-1 text-[9px] text-blue-700">Applies before {wave.month} recommendations refresh</span></> : <p className="text-[10px] text-gray-500 mt-2">Current store strategy continues unless you schedule a change for this wave.</p>}{waveLocked ? <p className="text-[10px] text-gray-500 mt-2 flex items-center gap-1"><Lock size={10} />Active waves cannot be changed.</p> : <button onClick={() => setStrategyEditorOpen(!strategyEditorOpen)} className="mt-3 text-[10px] font-medium text-blue-700 flex items-center gap-1"><Beaker size={11} />{scheduledStrategy ? 'Change scheduled strategy' : 'Schedule a strategy change'}</button>}</div>{strategyEditorOpen && !waveLocked && <div className="mt-3 rounded-xl border border-blue-200 bg-white p-3"><label className="text-[10px] font-medium text-gray-700">Scenario to apply<select className="mt-1.5 w-full rounded-lg border border-gray-200 bg-white px-2.5 py-2 text-[11px]"><option>Bring in more Spark Plugs</option><option>Reduce premium Brake Pad brands</option><option>Expand seasonal Wipers</option></select></label><p className="mt-2 text-[10px] text-gray-500">Pulse will simulate and re-rank this wave before it is locked.</p><div className="mt-3 flex gap-2"><button onClick={() => onNavigate('planner-door')} className="flex-1 rounded-lg border border-gray-200 px-2 py-2 text-[10px] font-medium">Open Planner</button><button onClick={scheduleStrategy} className="flex-1 rounded-lg bg-blue-700 text-white px-2 py-2 text-[10px] font-medium">Schedule for {wave.month.split(' ')[0]}</button></div></div>}<button onClick={() => onNavigate('planner-door')} className="mt-4 w-full rounded-lg border border-gray-200 px-3 py-2.5 text-xs font-medium flex justify-center items-center gap-2"><Beaker size={13} />View applied rules</button></aside>
      </div>

      <section className="mt-4 rounded-2xl border border-gray-200 bg-white overflow-hidden"><div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between"><div><h2 className="font-medium">Wave {wave.number} SKU recommendation impact</h2><p className="text-xs text-gray-500 mt-1">SKU counts are outcomes of the ${wave.returnBudget.toLocaleString()} return-value budget—not the constraint itself.</p></div><button onClick={() => onNavigate('sku-list')} className="text-xs text-blue-700 font-medium flex items-center gap-1">Review complete SKU list <ArrowRight size={13} /></button></div><div className="grid grid-cols-2 divide-x divide-gray-100"><div><div className="px-5 py-3 bg-green-50 text-xs font-medium text-green-900">{wave.orderUnits.toLocaleString()} highest-ranked missing SKUs to Order</div>{WAVE_ORDERS.slice(0,3).map(item => <div key={item.part} className="px-5 py-3 border-b border-gray-100 last:border-0 flex gap-3"><span className="text-[11px] font-semibold text-green-700 w-9">{item.rank}</span><div><div className="text-xs font-medium">{item.name}</div><div className="text-[10px] text-gray-500 mt-0.5">{item.part} · {item.reason}</div></div></div>)}</div><div><div className="px-5 py-3 bg-red-50 text-xs font-medium text-red-900">{wave.returnUnits.toLocaleString()} lowest-ranked SKUs to Return · ${wave.returnBudget.toLocaleString()} value</div>{WAVE_RETURNS.slice(0,3).map(item => <div key={item.part} className="px-5 py-3 border-b border-gray-100 last:border-0 flex gap-3"><span className="text-[11px] font-semibold text-red-700 w-14">{item.rank}</span><div><div className="text-xs font-medium">{item.name}</div><div className="text-[10px] text-gray-500 mt-0.5">{item.part} · {item.reason}</div></div></div>)}</div></div></section>
      <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 flex gap-3"><Clock3 size={16} className="text-amber-700 mt-0.5" /><p className="text-[11px] leading-relaxed text-amber-900"><strong>The budget controls return inventory value.</strong> Pulse fills each wave with the worst-ranked return recommendations until their value reaches the DC’s monthly P&amp;L limit. SKU quantity varies by item value; Orders are sequenced from the best missing products and are not counted against the return budget. Effective-dated Scenario Planner changes re-simulate future waves before lock; active waves stay frozen.</p></div>
    </div>
  );
}

const EXECUTION_LENSES = [
  { id: 'roadmap', label: 'Recovery roadmap', caption: 'Health and milestones', icon: Route },
  { id: 'categories', label: 'Category paths', caption: 'Search hierarchy impact', icon: BarChart3 },
  { id: 'budget', label: 'Budget utilization', caption: 'DC returns and P&L', icon: DollarSign },
  { id: 'skus', label: 'SKU actions', caption: 'Orders and Returns', icon: List },
];

function ExecutionLensRail({ lens, setLens }) {
  return <nav aria-label="Execution views" className="border-r border-gray-200 bg-gray-50/70 p-3">
    <div className="px-2 pt-1 pb-3 text-[9px] uppercase tracking-[0.16em] font-semibold text-gray-400">View plan as</div>
    <div className="space-y-1">{EXECUTION_LENSES.map(item => { const Icon = item.icon; const active = lens === item.id; return <button key={item.id} onClick={() => setLens(item.id)} className={`w-full rounded-xl px-3 py-3 text-left transition ${active ? 'bg-white border border-gray-200 shadow-sm text-gray-950' : 'border border-transparent text-gray-600 hover:bg-white/70'}`}><span className="flex items-center gap-2 text-xs font-medium"><Icon size={14} className={active ? 'text-blue-700' : 'text-gray-400'} />{item.label}</span><span className="mt-1 block pl-[22px] text-[9px] leading-4 text-gray-500">{item.caption}</span></button>; })}</div>
    <div className="mt-5 border-t border-gray-200 px-2 pt-4"><div className="flex items-center gap-2 text-[10px] font-medium text-gray-700"><RefreshCw size={11} className="text-blue-700" />Continuously refreshed</div><p className="mt-1 text-[9px] leading-4 text-gray-500">Future waves use the latest ranking and approved strategy.</p></div>
  </nav>;
}

function RecoveryRoadmapLens({ waves, selectedWave, setSelectedWave, scheduledStrategies }) {
  const firstAtRisk = waves.findIndex(item => item.after > 50);
  const firstExcellent = waves.findIndex(item => item.after > 70);
  const roadmapPoints = [{ number: 0, after: 29, month: 'Today' }, ...waves];
  const milestones = [
    { wave: 0, label: 'Execution begins', detail: `First $${waves[0]?.returnBudget.toLocaleString()} Return budget`, icon: PlayCircle },
    { wave: firstAtRisk, label: 'Store exits Critical', detail: waves[firstAtRisk]?.month, icon: TrendingUp },
    { wave: 8, label: 'Spark Plugs strategy', detail: 'Scheduled before May refresh', icon: CalendarDays },
    { wave: firstExcellent, label: 'Store reaches Excellent', detail: waves[firstExcellent]?.month, icon: CheckCircle2 },
  ].filter(item => item.wave >= 0 && item.wave < waves.length);

  return <div>
    <div className="rounded-xl border border-blue-100 bg-blue-50/70 px-4 py-3 flex gap-3"><Sparkles size={15} className="mt-0.5 text-blue-700 shrink-0" /><div><div className="text-xs font-medium text-blue-950">Pulse explanation</div><p className="mt-1 text-[11px] leading-5 text-blue-900">The first waves remove the lowest-ranked inventory first, creating the steepest health lift. The store exits Critical in {waves[firstAtRisk]?.month} and reaches Excellent in {waves[firstExcellent]?.month}. The May 2027 Spark Plugs strategy is applied before Wave 9 is rebuilt.</p></div></div>
    <div className="mt-4 rounded-xl border border-gray-200 overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between"><div><h3 className="text-sm font-medium">Projected health trajectory</h3><p className="text-[10px] text-gray-500 mt-0.5">Select any wave to inspect what changes.</p></div><HealthLegend /></div>
      <div className="relative h-[260px] px-3 pb-2 pt-3 bg-white">
        <div className="absolute left-14 right-3 top-3 h-[27%] rounded-t-lg bg-green-50/65 border-b border-green-100" />
        <div className="absolute left-14 right-3 top-[30%] h-[20%] bg-amber-50/60 border-b border-amber-100" />
        <div className="absolute left-14 right-3 top-[50%] bottom-8 rounded-b-lg bg-red-50/45" />
        <div className="absolute left-1 top-5 text-[9px] text-green-700">Excellent</div><div className="absolute left-1 top-[35%] text-[9px] text-amber-700">At risk</div><div className="absolute left-1 top-[69%] text-[9px] text-red-700">Critical</div>
        <div className="relative ml-12 h-full grid gap-1" style={{ gridTemplateColumns: `repeat(${roadmapPoints.length}, minmax(30px, 1fr))` }}>
          {roadmapPoints.map((item, pointIndex) => { const waveIndex = pointIndex - 1; const isToday = pointIndex === 0; const active = !isToday && selectedWave === waveIndex; const tone = pulseScoreTone(item.after); const strategy = !isToday && scheduledStrategies[waveIndex]; return <button key={`${item.number}-${item.month}`} disabled={isToday} onClick={() => !isToday && setSelectedWave(waveIndex)} aria-label={isToday ? 'Today health score 29' : `Wave ${item.number}, ${item.month}, projected health ${item.after}`} className={`relative h-full group ${isToday ? 'cursor-default' : 'cursor-pointer'}`}><span className="absolute inset-x-0 bottom-7"><span className={`mx-auto block w-[3px] rounded-t-full ${tone.bg} ${active ? 'opacity-100' : 'opacity-45 group-hover:opacity-75'}`} style={{ height: `${Math.max(38, item.after * 1.9)}px` }} /><span className={`absolute left-1/2 -translate-x-1/2 -top-2.5 w-3.5 h-3.5 rounded-full border-2 border-white ${tone.bg} ${active ? 'ring-2 ring-blue-500 ring-offset-2' : ''}`} /><strong className={`absolute left-1/2 -translate-x-1/2 -top-7 text-[10px] ${active ? 'text-blue-800' : tone.text}`}>{item.after}</strong>{strategy && <span className="absolute left-1/2 -translate-x-1/2 -top-12 w-5 h-5 rounded-md border border-blue-200 bg-blue-50 text-blue-700 flex items-center justify-center"><CalendarDays size={10} /></span>}</span><span className={`absolute inset-x-0 bottom-1 text-[8px] ${active ? 'font-semibold text-blue-800' : 'text-gray-500'}`}>{isToday ? 'Now' : item.month.split(' ')[0].slice(0,3)}</span></button>; })}
        </div>
      </div>
    </div>
    <div className="mt-4 grid grid-cols-2 gap-3">{milestones.map(item => { const Icon = item.icon; const active = selectedWave === item.wave; return <button key={`${item.wave}-${item.label}`} onClick={() => setSelectedWave(item.wave)} className={`rounded-xl border px-3 py-3 text-left flex items-start gap-3 ${active ? 'border-blue-300 bg-blue-50' : 'border-gray-200 bg-white hover:border-blue-200'}`}><span className={`mt-0.5 w-7 h-7 rounded-lg flex items-center justify-center ${active ? 'bg-blue-700 text-white' : 'bg-gray-100 text-gray-500'}`}><Icon size={13} /></span><span><strong className="block text-[11px]">Wave {item.wave + 1} · {item.label}</strong><small className="mt-0.5 block text-[9px] text-gray-500">{item.detail}</small></span></button>; })}</div>
  </div>;
}

function CategoryRecoveryLens({ waves, selectedWave, setSelectedWave, onNavigate }) {
  const categories = waves[0]?.categories || [];
  return <div>
    <div className="flex items-start justify-between"><div><h3 className="text-sm font-medium">When categories get well</h3><p className="text-[10px] text-gray-500 mt-1">Each cell is the projected category status after that wave.</p></div><HealthLegend /></div>
    <div className="mt-4 rounded-xl border border-gray-200 overflow-hidden"><div className="grid bg-gray-50 border-b border-gray-200 px-2 py-2 text-[8px] uppercase tracking-wider text-gray-500" style={{gridTemplateColumns:'120px repeat(15,minmax(20px,1fr))'}}><span>Category</span>{waves.map(item => <span key={item.number} className="text-center">W{item.number}</span>)}</div>{categories.map((category, categoryIndex) => { const rowScores = waves.map(item => item.categories[categoryIndex]); const atRiskIndex = rowScores.findIndex(item => item.after > 50); const excellentIndex = rowScores.findIndex(item => item.after > 70); return <div key={category.name} className="grid items-stretch border-b border-gray-100 last:border-0 px-2" style={{gridTemplateColumns:'120px repeat(15,minmax(20px,1fr))'}}><button onClick={() => onNavigate('category-l2')} className="py-3 pr-2 text-left"><strong className="block text-[9px] leading-4">{category.name}</strong><small className="text-[8px] text-gray-500">{category.before} today</small></button>{rowScores.map((cell,index) => { const tone = pulseScoreTone(cell.after); const crossing = index === atRiskIndex ? 'A' : index === excellentIndex ? 'E' : ''; return <button key={index} onClick={() => setSelectedWave(index)} aria-label={`${category.name}, Wave ${index + 1}, score ${cell.after}`} className={`m-0.5 rounded-md border flex flex-col items-center justify-center ${tone.soft} ${selectedWave === index ? 'ring-2 ring-blue-500 ring-offset-1' : ''}`}><span className={`text-[8px] font-semibold ${tone.text}`}>{cell.after}</span>{crossing && <span className="text-[6px] font-bold text-gray-500">{crossing}</span>}</button>; })}</div>; })}</div>
    <div className="mt-3 flex items-center justify-between text-[9px] text-gray-500"><span><strong>A</strong> = enters At risk · <strong>E</strong> = enters Excellent</span><button onClick={() => onNavigate('category-l2')} className="text-blue-700 font-medium flex items-center gap-1">Open category hierarchy <ArrowRight size={11} /></button></div>
  </div>;
}

function BudgetUtilizationLens({ waves, budget, selectedWave, setSelectedWave, budgetEditing, setBudgetEditing, setBudget }) {
  const wave = waves[selectedWave];
  const cumulative = RETURN_VALUE_BACKLOG - wave.remainingReturnValue;
  const utilization = Math.min(100, Math.round((wave.returnBudget / Number(budget || 1)) * 100));
  return <div>
    <div className="grid grid-cols-2 gap-3"><section className="rounded-xl border border-red-100 bg-red-50/60 p-4"><div className="flex items-center justify-between"><span className="text-[10px] font-medium text-red-800">DC Return track</span><span className="text-[9px] text-red-700">Counts against budget</span></div><div className="mt-2 flex items-end gap-2"><strong className="text-2xl">${wave.returnBudget.toLocaleString()}</strong><span className="pb-1 text-[10px] text-gray-500">of ${Number(budget).toLocaleString()}</span></div><div className="mt-3 h-2 rounded-full bg-white overflow-hidden"><div className="h-full rounded-full bg-red-600" style={{width:`${utilization}%`}} /></div><p className="mt-2 text-[10px] text-gray-600">{wave.returnUnits.toLocaleString()} SKUs selected by lowest rank until the value cap is reached.</p></section><section className="rounded-xl border border-green-100 bg-green-50/60 p-4"><div className="flex items-center justify-between"><span className="text-[10px] font-medium text-green-800">Store Order track</span><span className="text-[9px] text-green-700">Outside return budget</span></div><strong className="mt-2 block text-2xl">{wave.orderUnits.toLocaleString()} SKUs</strong><div className="mt-3 h-2 rounded-full bg-white overflow-hidden"><div className="h-full rounded-full bg-green-600" style={{width:`${Math.min(100, 42 + selectedWave * 3)}%`}} /></div><p className="mt-2 text-[10px] text-gray-600">Highest-ranked missing products sequenced alongside this wave.</p></section></div>
    <section className="mt-4 rounded-xl border border-gray-200 p-4"><div className="flex items-start justify-between"><div><h3 className="text-sm font-medium">Monthly return-value plan</h3><p className="mt-1 text-[10px] text-gray-500">$148,600 is recovered over {waves.length} waves. SKU quantity varies with item value.</p></div><button onClick={() => setBudgetEditing(!budgetEditing)} className="rounded-lg border border-gray-200 px-3 py-2 text-[10px] font-medium text-blue-700"><PencilLine size={11} className="inline mr-1" />Edit budget</button></div>{budgetEditing && <div className="mt-3 rounded-lg border border-blue-100 bg-blue-50 p-3 flex items-center gap-3"><label className="text-[10px] text-blue-900">Monthly return budget<div className="mt-1 flex items-center rounded-lg border border-blue-200 bg-white px-3"><span className="text-xs">$</span><input type="number" min="1000" step="1000" value={budget} onChange={event => setBudget(event.target.value)} className="w-28 bg-transparent px-1 py-2 text-sm font-semibold outline-none" /></div></label><p className="text-[10px] leading-4 text-blue-800">Changing the budget recalculates the timing, Return SKU count, and projected health path.</p></div>}
      <div className="mt-5 h-40 flex items-end gap-1.5 border-b border-gray-200 px-1">{waves.map((item,index) => { const fill = Math.max(8, Math.round((item.returnBudget / Number(budget || 1)) * 112)); const active = selectedWave === index; return <button key={item.number} onClick={() => setSelectedWave(index)} className="relative flex-1 h-full flex items-end justify-center group" aria-label={`Wave ${item.number}, ${item.month}, return value ${item.returnBudget}`}><span className={`w-full max-w-6 rounded-t ${active ? 'bg-blue-700' : 'bg-blue-200 group-hover:bg-blue-300'}`} style={{height:`${fill}px`}} /><span className={`absolute -bottom-5 text-[8px] ${active ? 'font-semibold text-blue-700' : 'text-gray-500'}`}>W{item.number}</span></button>; })}</div>
      <div className="mt-8 grid grid-cols-3 gap-3"><div><span className="text-[9px] text-gray-500">Through selected wave</span><strong className="block mt-1 text-sm">${cumulative.toLocaleString()}</strong></div><div><span className="text-[9px] text-gray-500">Return value remaining</span><strong className="block mt-1 text-sm">${wave.remainingReturnValue.toLocaleString()}</strong></div><div><span className="text-[9px] text-gray-500">Projected completion</span><strong className="block mt-1 text-sm">{waves[waves.length-1].month}</strong></div></div>
    </section>
  </div>;
}

function SkuActionsLens({ wave, onNavigate }) {
  return <div><div className="rounded-xl border border-blue-100 bg-blue-50/70 px-4 py-3 flex items-start gap-3"><Info size={14} className="mt-0.5 text-blue-700" /><p className="text-[11px] leading-5 text-blue-900"><strong>Two coordinated tracks, not a swap.</strong> Return quantity is determined by the ${wave.returnBudget.toLocaleString()} DC value limit. Orders come from the highest-ranked missing products and do not consume that budget.</p></div><div className="mt-4 grid grid-cols-2 gap-3"><section className="rounded-xl border border-green-100 overflow-hidden"><div className="bg-green-50 px-4 py-3"><div className="text-[9px] uppercase tracking-wider text-green-700">Order recommendations</div><strong className="mt-1 block text-lg text-green-950">{wave.orderUnits.toLocaleString()} SKUs</strong><span className="text-[9px] text-green-800">Highest-ranked missing products</span></div><div className="divide-y divide-gray-100 bg-white">{WAVE_ORDERS.map(item => <div key={item.part} className="px-4 py-3 flex gap-3"><span className="text-[10px] font-semibold text-green-700 w-7">{item.rank}</span><span><strong className="block text-[11px]">{item.name}</strong><small className="text-[9px] text-gray-500">{item.part} · {item.reason}</small></span></div>)}</div></section><section className="rounded-xl border border-red-100 overflow-hidden"><div className="bg-red-50 px-4 py-3"><div className="text-[9px] uppercase tracking-wider text-red-700">Return recommendations</div><strong className="mt-1 block text-lg text-red-950">{wave.returnUnits.toLocaleString()} SKUs · ${wave.returnBudget.toLocaleString()}</strong><span className="text-[9px] text-red-800">Lowest-ranked products held</span></div><div className="divide-y divide-gray-100 bg-white">{WAVE_RETURNS.map(item => <div key={item.part} className="px-4 py-3 flex gap-3"><span className="text-[10px] font-semibold text-red-700 w-12">{item.rank}</span><span><strong className="block text-[11px]">{item.name}</strong><small className="text-[9px] text-gray-500">{item.part} · {item.reason}</small></span></div>)}</div></section></div><button onClick={() => onNavigate('sku-list')} className="mt-4 w-full rounded-xl border border-blue-200 bg-white px-4 py-3 text-xs font-medium text-blue-700 flex items-center justify-center gap-2"><List size={13} />Review the complete scoped SKU list</button></div>;
}

function PulseStoreWorkspaceScreenV1({ onNavigate }) {
  const [view, setView] = useState('overview');
  const [lens, setLens] = useState('roadmap');
  const [selectedWave, setSelectedWave] = useState(0);
  const [launched, setLaunched] = useState(false);
  const [budget, setBudget] = useState(10000);
  const [budgetEditing, setBudgetEditing] = useState(false);
  const [strategyEditorOpen, setStrategyEditorOpen] = useState(false);
  const [scenarioChoice, setScenarioChoice] = useState('Expand Spark Plugs assortment');
  const [scheduledStrategies, setScheduledStrategies] = useState(() => ({ 8: { title: 'Expand Spark Plugs assortment', detail: 'Bring in more top-ranked Spark Plugs before this wave is built.', status: 'Scheduled' } }));
  const waves = useMemo(() => buildReturnBudgetJourney(budget), [budget]);
  const safeSelectedWave = Math.min(selectedWave, waves.length - 1);
  const wave = waves[safeSelectedWave];
  const scheduledStrategy = scheduledStrategies[safeSelectedWave];
  const waveLocked = safeSelectedWave === 0 && launched;
  const stageIndex = waveLocked ? 3 : safeSelectedWave === 0 ? 1 : 0;

  const scheduleStrategy = () => {
    if (waveLocked) return;
    setScheduledStrategies(previous => ({ ...previous, [safeSelectedWave]: { title: scenarioChoice, detail: scenarioChoice === 'Expand Spark Plugs assortment' ? 'Bring in more top-ranked Spark Plugs before this wave is built.' : 'Re-rank this wave using the newly approved assortment rule.', status: 'Scheduled' } }));
    setStrategyEditorOpen(false);
  };
  ...
  return <div>
    <Breadcrumb items={['Assortment overview', 'Store health', 'ATL-050', 'Store execution']} />
    <div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2"><h1 className="text-[22px] font-medium">ATL-050 (DOWNTOWN ATLANTA)</h1><span className={`rounded-full border px-2.5 py-1 text-[10px] ${launched ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-amber-50 border-amber-200 text-amber-800'}`}>{launched ? 'Wave 1 in progress' : 'Plan ready for review'}</span></div><p className="text-sm text-gray-500 mt-1">A staged path from today’s assortment to a healthier store.</p></div><div className="flex items-center gap-2"><button onClick={() => onNavigate('sku-list')} className="rounded-full border border-blue-700 text-blue-700 bg-white px-4 py-2 text-xs flex items-center gap-2"><List size={14} />SKU list</button><button onClick={() => { setSelectedWave(0); setLaunched(true); }} className="rounded-full bg-blue-700 hover:bg-blue-800 text-white px-4 py-2 text-xs font-medium flex items-center gap-2"><Lock size={13} />{launched ? 'Wave 1 locked' : 'Review & lock Wave 1'}</button></div></div>
    <div className="mt-4 flex items-center justify-between"><StorePageTabs view={view} setView={setView} /><div className="flex items-center gap-2 text-xs text-gray-500"><RefreshCw size={13} />Recommendations refreshed today · future waves remain dynamic</div></div>

    <section className="mt-4 rounded-2xl border border-gray-200 bg-white overflow-hidden"><div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between"><div className="flex items-center gap-3"><span className="w-8 h-8 rounded-lg bg-blue-950 text-white flex items-center justify-center"><Route size={15} /></span><div><div className="text-xs font-medium">Recovery plan</div><p className="text-[10px] text-gray-500 mt-0.5">One recommendation set · four synchronized views</p></div></div><button onClick={() => onNavigate('planner-door')} className="text-[10px] font-medium text-blue-700 flex items-center gap-1"><Beaker size={12} />ATL-050 Q4 strategy · 4 rules <ArrowRight size={11} /></button></div><div className="grid grid-cols-4 divide-x divide-gray-100"><div className="px-5 py-3"><span className="text-[10px] text-gray-500">Health journey</span><div className="mt-1 flex items-center gap-1.5"><strong className="text-xl">29</strong><ArrowRight size={13} className="text-gray-400" /><strong className="text-xl text-green-700">91</strong></div></div><div className="px-5 py-3"><span className="text-[10px] text-gray-500">Return value pending</span><strong className="text-xl block mt-1">$148,600</strong></div><div className="px-5 py-3"><span className="text-[10px] text-gray-500">Monthly DC return budget</span><button onClick={() => { setLens('budget'); setBudgetEditing(true); }} className="mt-1 flex items-center gap-2"><strong className="text-xl">${Number(budget).toLocaleString()}</strong><PencilLine size={11} className="text-blue-700" /></button></div><div className="px-5 py-3"><span className="text-[10px] text-gray-500">Projected recovery</span><strong className="text-xl block mt-1">{waves.length} waves</strong><small className="text-[9px] text-gray-500">Complete {waves[waves.length - 1].month}</small></div></div></section>

    <section className="mt-4 rounded-2xl border border-gray-200 bg-white overflow-hidden"><div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between"><div><h2 className="text-sm font-medium">Store execution workspace</h2><p className="mt-0.5 text-[10px] text-gray-500">Select a view, then click any point to inspect the same wave.</p></div><div className="flex items-center gap-2 text-[9px] text-gray-500"><span className="w-2 h-2 rounded-full bg-blue-600" />Selected: Wave {wave.number} · {wave.month}</div></div><div className="grid" style={{gridTemplateColumns:'155px minmax(0,1fr) 285px'}}><ExecutionLensRail lens={lens} setLens={setLens} /><main className="p-4 min-h-[565px]">{lens === 'roadmap' && <RecoveryRoadmapLens waves={waves} selectedWave={safeSelectedWave} setSelectedWave={setSelectedWave} scheduledStrategies={scheduledStrategies} />}{lens === 'categories' && <CategoryRecoveryLens waves={waves} selectedWave={safeSelectedWave} setSelectedWave={setSelectedWave} onNavigate={onNavigate} />}{lens === 'budget' && <BudgetUtilizationLens waves={waves} budget={budget} selectedWave={safeSelectedWave} setSelectedWave={setSelectedWave} budgetEditing={budgetEditing} setBudgetEditing={setBudgetEditing} setBudget={setBudget} />}{lens === 'skus' && <SkuActionsLens wave={wave} onNavigate={onNavigate} />}</main><aside className="border-l border-gray-200 bg-gray-50/45 p-4"><div className="flex items-start justify-between"><div><div className="text-[9px] uppercase tracking-[0.14em] text-gray-500">Selected wave</div><h3 className="mt-1 text-sm font-medium">Wave {wave.number} · {wave.month}</h3></div><span className={`rounded-full border px-2 py-1 text-[8px] ${waveLocked ? 'border-blue-200 bg-blue-50 text-blue-700' : 'border-amber-200 bg-amber-50 text-amber-800'}`}>{waveLocked ? 'In progress' : safeSelectedWave === 0 ? 'Needs review' : 'Draft'}</span></div>
      <div className="mt-4 grid grid-cols-4 gap-1">{['Draft','Review','Locked','Executing'].map((stage,index) => <div key={stage}><span className={`block h-1 rounded-full ${index <= stageIndex ? 'bg-blue-700' : 'bg-gray-200'}`} /><span className={`mt-1 block text-[7px] ${index === stageIndex ? 'font-semibold text-blue-700' : 'text-gray-400'}`}>{stage}</span></div>)}</div>
      <div className="mt-4 rounded-xl border border-gray-200 bg-white p-3"><div className="text-[9px] text-gray-500">Projected health</div><div className="mt-1 flex items-center gap-2"><strong className="text-2xl">{wave.before}</strong><ArrowRight size={14} className="text-gray-400" /><strong className={`text-2xl ${pulseScoreTone(wave.after).text}`}>{wave.after}</strong><span className="ml-auto text-[9px] text-green-700">+{wave.newlyAligned.toLocaleString()} aligned</span></div></div>
      <div className="mt-3 grid grid-cols-2 gap-2"><button onClick={() => setLens('budget')} className="rounded-xl border border-red-100 bg-red-50 p-3 text-left"><span className="text-[8px] uppercase tracking-wider text-red-700">DC Returns</span><strong className="mt-1 block text-sm">${wave.returnBudget.toLocaleString()}</strong><small className="text-[8px] text-red-800">{wave.returnUnits.toLocaleString()} SKUs · budgeted</small></button><button onClick={() => setLens('skus')} className="rounded-xl border border-green-100 bg-green-50 p-3 text-left"><span className="text-[8px] uppercase tracking-wider text-green-700">Store Orders</span><strong className="mt-1 block text-sm">{wave.orderUnits.toLocaleString()} SKUs</strong><small className="text-[8px] text-green-800">Outside return budget</small></button></div>
      <dl className="mt-4 space-y-2 text-[10px]"><div className="flex justify-between"><dt className="text-gray-500">Return value remaining</dt><dd className="font-semibold">${wave.remainingReturnValue.toLocaleString()}</dd></div><div className="flex justify-between"><dt className="text-gray-500">Strategy-affected SKUs</dt><dd className="font-semibold">{wave.strategySkus.toLocaleString()}</dd></div></dl>
      <div className={`mt-4 rounded-xl border p-3 ${scheduledStrategy ? 'border-blue-200 bg-blue-50' : 'border-gray-200 bg-white'}`}><div className="flex items-center gap-2 text-[10px] font-medium"><CalendarDays size={12} className={scheduledStrategy ? 'text-blue-700' : 'text-gray-500'} />Strategy before this wave</div>{scheduledStrategy ? <><strong className="mt-2 block text-[10px] text-blue-950">{scheduledStrategy.title}</strong><p className="mt-1 text-[9px] leading-4 text-blue-800">Applies before {wave.month} recommendations refresh.</p></> : <p className="mt-2 text-[9px] leading-4 text-gray-500">Current strategy continues unless a change is scheduled.</p>}{waveLocked ? <p className="mt-2 flex items-center gap-1 text-[8px] text-gray-500"><Lock size={9} />Active wave is immutable.</p> : <button onClick={() => setStrategyEditorOpen(!strategyEditorOpen)} className="mt-2 text-[9px] font-medium text-blue-700">{scheduledStrategy ? 'Change scheduled strategy' : 'Schedule strategy change'}</button>}</div>
      {strategyEditorOpen && !waveLocked && <div className="mt-3 rounded-xl border border-blue-200 bg-white p-3"><label className="text-[9px] font-medium text-gray-700">Scenario to apply<select value={scenarioChoice} onChange={event => setScenarioChoice(event.target.value)} className="mt-1.5 w-full rounded-lg border border-gray-200 bg-white px-2 py-2 text-[9px]"><option>Expand Spark Plugs assortment</option><option>Reduce premium Brake Pad brands</option><option>Expand seasonal Wipers</option></select></label><p className="mt-2 text-[8px] leading-4 text-gray-500">Pulse will simulate and re-rank this wave before lock.</p><button onClick={scheduleStrategy} className="mt-2 w-full rounded-lg bg-blue-700 px-2 py-2 text-[9px] font-medium text-white">Schedule for {wave.month.split(' ')[0]}</button></div>}
      <div className="mt-4 rounded-xl border border-blue-100 bg-blue-50/70 p-3"><div className="flex gap-2"><Sparkles size={12} className="mt-0.5 text-blue-700 shrink-0" /><p className="text-[9px] leading-4 text-blue-900">Wave {wave.number} uses ${wave.returnBudget.toLocaleString()} to remove {wave.returnUnits.toLocaleString()} low-ranked SKUs and recommends {wave.orderUnits.toLocaleString()} missing products, moving store health to {wave.after}.</p></div></div>
      <button onClick={() => { if (safeSelectedWave === 0) setLaunched(true); }} className={`mt-4 w-full rounded-xl px-3 py-3 text-[10px] font-medium flex items-center justify-center gap-2 ${waveLocked ? 'border border-blue-200 bg-blue-50 text-blue-700' : 'bg-blue-700 text-white'}`}>{waveLocked ? <><CheckCircle2 size={12} />Wave locked · executing</> : safeSelectedWave === 0 ? <><Lock size={12} />Review & lock this wave</> : <><Eye size={12} />Review future wave</>}</button><p className="mt-2 text-center text-[8px] leading-4 text-gray-500">Nothing is exported to the DC without review.</p>
    </aside></div></section>
    <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 flex gap-3"><Clock3 size={15} className="mt-0.5 text-amber-700" /><p className="text-[10px] leading-5 text-amber-900"><strong>Operational guardrail:</strong> locked waves remain stable. Daily ranking changes and effective-dated Scenario Planner rules only rebuild future waves; Orders remain separate from the DC return-value budget.</p></div>
  </div>;
}

const EXECUTION_CATEGORY_PATHS = [
  { path: ['Brakes', 'Disc Brake', 'Brake Pads', 'Ceramic'], before: 18, lift: 12, skuCount: 146, returnCount: 38, orderCount: 108 },
  { path: ['Engine Parts', 'Ignition', 'Spark Plugs', 'Iridium'], before: 24, lift: 11, skuCount: 132, returnCount: 22, orderCount: 110 },
  { path: ['Body & Accessories', 'Interior', 'Hardware & Components', 'BK – Balkamp'], before: 30, lift: 8, skuCount: 94, returnCount: 71, orderCount: 23 },
  { path: ['Climate Control', 'A/C Service', 'Compressors', 'Remanufactured'], before: 38, lift: 7, skuCount: 81, returnCount: 44, orderCount: 37 },
  { path: ['Steering & Suspension', 'Chassis', 'Control Arms', 'Front'], before: 32, lift: 6, skuCount: 76, returnCount: 29, orderCount: 47 },
  { path: ['Tools & Equipment', 'Shop Equipment', 'Diagnostics', 'Electrical'], before: 35, lift: 5, skuCount: 61, returnCount: 33, orderCount: 28 },
];

function WaveNavigator({ waves, selectedWave, setSelectedWave }) {
  const wave = waves[selectedWave];
  return <div className="rounded-xl border border-gray-200 bg-gray-50/70 p-3">
    <div className="flex items-center gap-3">
      <button disabled={selectedWave === 0} onClick={() => setSelectedWave(selectedWave - 1)} className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-[10px] font-medium text-gray-700 disabled:opacity-35"><ArrowLeft size={12} className="inline mr-1" />Previous</button>
      <label className="flex items-center gap-2 text-[10px] font-medium text-gray-600">Jump to
        <select aria-label="Select execution wave" value={selectedWave} onChange={event => setSelectedWave(Number(event.target.value))} className="min-w-[190px] rounded-lg border border-blue-200 bg-white px-3 py-2 text-[11px] font-semibold text-blue-900 outline-none">
          {waves.map((item,index) => <option key={item.number} value={index}>Wave {item.number} — {item.month} — health {item.after}</option>)}
        </select>
      </label>
      <button disabled={selectedWave === waves.length - 1} onClick={() => setSelectedWave(selectedWave + 1)} className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-[10px] font-medium text-gray-700 disabled:opacity-35">Next<ArrowRight size={12} className="inline ml-1" /></button>
      <div className="ml-auto text-right"><div className="text-[9px] text-gray-500">Currently inspecting</div><strong className="text-[11px]">Wave {wave.number} · {wave.month}</strong></div>
    </div>
    <div className="mt-3 grid gap-1" style={{gridTemplateColumns:`repeat(${waves.length},minmax(34px,1fr))`}}>
      {waves.map((item,index) => <button key={item.number} onClick={() => setSelectedWave(index)} aria-label={`Select Wave ${item.number}, ${item.month}`} className={`rounded-lg border px-1 py-2 text-center transition ${selectedWave === index ? 'border-blue-700 bg-blue-700 text-white shadow-sm' : 'border-gray-200 bg-white text-gray-600 hover:border-blue-300'}`}><strong className="block text-[10px]">W{item.number}</strong><span className={`mt-0.5 block text-[7px] ${selectedWave === index ? 'text-blue-100' : 'text-gray-400'}`}>{item.month.split(' ')[0].slice(0,3)}</span></button>)}
    </div>
  </div>;
}

function WaveSkuPreview({ wave, waveIndex, onNavigate }) {
  const [actionFilter, setActionFilter] = useState('All actions');
  const [skuQuery, setSkuQuery] = useState('');
  const categoryPaths = EXECUTION_CATEGORY_PATHS;
  const returns = WAVE_RETURNS.map((item,index) => ({...item, action:'Return', value:[184,96,128,72][index], path:categoryPaths[(index + waveIndex) % categoryPaths.length].path}));
  const orders = WAVE_ORDERS.map((item,index) => ({...item, action:'Order', value:null, path:categoryPaths[(index + waveIndex + 1) % categoryPaths.length].path}));
  const rows = [...returns.slice(0,3), ...orders.slice(0,3)].filter(item => (actionFilter === 'All actions' || item.action === actionFilter) && `${item.part} ${item.name} ${item.brand} ${item.path.join(' ')}`.toLowerCase().includes(skuQuery.toLowerCase()));
  return <section className="rounded-xl border border-gray-200 bg-white overflow-hidden">
    <div className="px-4 py-3 border-b border-gray-100 flex items-start justify-between gap-3"><div><h3 className="text-sm font-medium">SKU actions in Wave {wave.number}</h3><p className="mt-1 text-[10px] text-gray-500">A working preview of what Pulse will execute—not a separate view to discover later.</p></div><div className="text-right"><strong className="block text-[11px]">{(wave.returnUnits + wave.orderUnits).toLocaleString()} actions</strong><span className="text-[9px] text-gray-500">{wave.returnUnits.toLocaleString()} Returns · {wave.orderUnits.toLocaleString()} Orders</span></div></div>
    <div className="px-4 py-3 flex items-center justify-between gap-3 bg-gray-50/60"><label className="relative flex-1 max-w-[300px]"><Search size={12} className="absolute left-3 top-2.5 text-gray-400" /><input value={skuQuery} onChange={event => setSkuQuery(event.target.value)} placeholder="Search this wave’s SKUs" className="w-full rounded-lg border border-gray-200 bg-white py-2 pl-8 pr-3 text-[10px] outline-none focus:border-blue-500" /></label><div className="inline-flex rounded-lg border border-gray-200 bg-white p-0.5">{['All actions','Return','Order'].map(item => <button key={item} onClick={() => setActionFilter(item)} className={`rounded-md px-3 py-1.5 text-[9px] font-medium ${actionFilter === item ? 'bg-gray-950 text-white' : 'text-gray-600'}`}>{item}</button>)}</div></div>
    <div className="grid bg-gray-50 px-4 py-2 text-[8px] uppercase tracking-wider text-gray-500" style={{gridTemplateColumns:'66px 70px 1fr 1.25fr 65px'}}><span>Action</span><span>Rank</span><span>SKU</span><span>Category path</span><span>Value</span></div>
    <div className="divide-y divide-gray-100">{rows.map(item => <button key={`${item.action}-${item.part}`} onClick={() => onNavigate('sku-list')} className="w-full grid items-center px-4 py-3 text-left hover:bg-blue-50/35" style={{gridTemplateColumns:'66px 70px 1fr 1.25fr 65px'}}><span><i className={`not-italic rounded-full border px-2 py-1 text-[8px] font-semibold ${item.action === 'Return' ? 'border-red-200 bg-red-50 text-red-700' : 'border-green-200 bg-green-50 text-green-700'}`}>{item.action}</i></span><strong className={`text-[9px] ${item.action === 'Return' ? 'text-red-700' : 'text-green-700'}`}>{item.rank}</strong><span><strong className="block text-[10px]">{item.name}</strong><small className="text-[8px] text-gray-500">{item.part} · {item.brand}</small></span><span className="pr-3 text-[8px] leading-4 text-gray-600">{item.path.join(' › ')}</span><strong className="text-[9px]">{item.value ? `$${item.value}` : '—'}</strong></button>)}</div>
    <div className="border-t border-gray-100 bg-gray-50 px-4 py-3 flex items-center justify-between"><span className="text-[9px] text-gray-500">Showing representative actions from the selected wave</span><button onClick={() => onNavigate('sku-list')} className="text-[10px] font-medium text-blue-700 flex items-center gap-1">Review all Wave {wave.number} SKUs <ArrowRight size={11} /></button></div>
  </section>;
}

function WaveCategoryExplorer({ wave, waveIndex, onNavigate }) {
  const [categoryQuery, setCategoryQuery] = useState('');
  const paths = EXECUTION_CATEGORY_PATHS.map((item,index) => ({...item, after:Math.min(96,item.before + item.lift + Math.round(waveIndex * (2.8 - index * .18)))})).filter(item => item.path.join(' ').toLowerCase().includes(categoryQuery.toLowerCase()));
  return <section className="rounded-xl border border-gray-200 bg-white overflow-hidden">
    <div className="px-4 py-3 border-b border-gray-100 flex items-start justify-between"><div><h3 className="text-sm font-medium">Category paths affected in Wave {wave.number}</h3><p className="mt-1 text-[10px] text-gray-500">Searchable paths replace the all-category matrix. Pulse monitors 1,487 hierarchy nodes across L1–L4.</p></div><button onClick={() => onNavigate('category-l2')} className="text-[10px] font-medium text-blue-700 flex items-center gap-1">Open full hierarchy <ArrowRight size={11} /></button></div>
    <div className="px-4 py-3 bg-gray-50/60"><label className="relative block max-w-sm"><Search size={12} className="absolute left-3 top-2.5 text-gray-400" /><input value={categoryQuery} onChange={event => setCategoryQuery(event.target.value)} placeholder="Search L1, L2, L3, L4 or brand" className="w-full rounded-lg border border-gray-200 bg-white py-2 pl-8 pr-3 text-[10px] outline-none focus:border-blue-500" /></label></div>
    <div className="divide-y divide-gray-100">{paths.map(item => { const before = Math.min(item.after - 1,item.before + Math.round(waveIndex * 2.2)); const tone = pulseScoreTone(item.after); return <button key={item.path.join('-')} onClick={() => onNavigate('category-l2')} className="w-full px-4 py-3 text-left hover:bg-blue-50/35"><div className="flex items-center gap-2 text-[8px] text-gray-500">{item.path.map((node,index) => <React.Fragment key={node}><span className={index === item.path.length - 1 ? 'font-semibold text-gray-800' : ''}>{node}</span>{index < item.path.length - 1 && <ChevronRight size={9} />}</React.Fragment>)}</div><div className="mt-2 grid items-center gap-3" style={{gridTemplateColumns:'1fr 95px 125px'}}><div className="h-1.5 rounded-full bg-gray-100 overflow-hidden"><div className={`${tone.bg} h-full rounded-full`} style={{width:`${item.after}%`}} /></div><strong className={`text-[10px] ${tone.text}`}>{before} → {item.after}</strong><span className="text-right text-[8px] text-gray-500">{item.returnCount} Returns · {item.orderCount} Orders</span></div></button>; })}</div>
    <div className="border-t border-gray-100 bg-gray-50 px-4 py-3 text-[9px] text-gray-500">Showing {paths.length} highest-impact paths for this wave. Search or open the existing category drill-down to navigate the complete hierarchy.</div>
  </section>;
}

function PulseStoreWorkspaceScreenV2({ onNavigate }) {
  const [view, setView] = useState('overview');
  const [selectedWave, setSelectedWave] = useState(0);
  const [launched, setLaunched] = useState(false);
  const [budget, setBudget] = useState(10000);
  ...
  const scheduledStrategy = scheduledStrategies[safeSelectedWave];
  const waveLocked = safeSelectedWave === 0 && launched;
  const scheduleStrategy = () => { if (waveLocked) return; setScheduledStrategies(previous => ({...previous,[safeSelectedWave]:{title:scenarioChoice,detail:'Pulse will apply this approved rule before rebuilding the wave recommendations.',status:'Scheduled'}})); setStrategyEditorOpen(false); };
  ...
  return <div>
    <Breadcrumb items={['Assortment overview','Store health','ATL-050','Store execution']} />
    <div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2"><h1 className="text-[22px] font-medium">ATL-050 (DOWNTOWN ATLANTA)</h1><span className={`rounded-full border px-2.5 py-1 text-[10px] ${launched ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-amber-50 border-amber-200 text-amber-800'}`}>{launched ? 'Wave 1 in progress' : 'Plan ready for review'}</span></div><p className="mt-1 text-sm text-gray-500">See the recovery path, then inspect the exact SKU and category decisions inside every wave.</p></div><button onClick={() => onNavigate('sku-list')} className="rounded-full border border-blue-700 bg-white px-4 py-2 text-xs text-blue-700 flex items-center gap-2"><List size={14} />Complete SKU list</button></div>
    <div className="mt-4 flex items-center justify-between"><StorePageTabs view={view} setView={setView} /><div className="flex items-center gap-2 text-xs text-gray-500"><RefreshCw size={13} />Recommendations refreshed today · future waves remain dynamic</div></div>

    <section className="mt-4 rounded-2xl border border-gray-200 bg-white overflow-hidden"><div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between"><div className="flex items-center gap-3"><span className="w-8 h-8 rounded-lg bg-blue-950 text-white flex items-center justify-center"><Route size={15} /></span><div><div className="text-xs font-medium">Store recovery plan</div><p className="mt-0.5 text-[10px] text-gray-500">One continuous journey · 15 inspectable monthly waves</p></div></div><button onClick={() => onNavigate('planner-door')} className="text-[10px] font-medium text-blue-700 flex items-center gap-1"><Beaker size={12} />ATL-050 Q4 strategy · 4 rules <ArrowRight size={11} /></button></div><div className="grid grid-cols-4 divide-x divide-gray-100"><div className="px-5 py-3"><span className="text-[10px] text-gray-500">Health journey</span><div className="mt-1 flex items-center gap-1.5"><strong className="text-xl">29</strong><ArrowRight size={13} className="text-gray-400" /><strong className="text-xl text-green-700">91</strong></div></div><div className="px-5 py-3"><span className="text-[10px] text-gray-500">Return value pending</span><strong className="mt-1 block text-xl">$148,600</strong></div><div className="px-5 py-3"><span className="text-[10px] text-gray-500">Monthly DC return budget</span><button onClick={() => setBudgetEditing(!budgetEditing)} className="mt-1 flex items-center gap-2"><strong className="text-xl">${Number(budget).toLocaleString()}</strong><PencilLine size={11} className="text-blue-700" /></button></div><div className="px-5 py-3"><span className="text-[10px] text-gray-500">Projected recovery</span><strong className="mt-1 block text-xl">{waves.length} waves</strong><small className="text-[9px] text-gray-500">Complete {waves[waves.length - 1].month}</small></div></div>{budgetEditing && <div className="border-t border-blue-100 bg-blue-50 px-5 py-3 flex items-center gap-3"><label className="text-[10px] text-blue-900">Monthly return-value budget <span className="ml-2 rounded-lg border border-blue-200 bg-white px-2 py-1"><span>$</span><input type="number" min="1000" step="1000" value={budget} onChange={event => setBudget(event.target.value)} className="w-24 px-1 text-xs font-semibold outline-none" /></span></label><span className="text-[9px] text-blue-800">Changing the budget recalculates future wave timing and SKU quantities.</span></div>}</section>

    <section className="mt-4 rounded-2xl border border-gray-200 bg-white p-4"><div className="flex items-start justify-between mb-3"><div><h2 className="text-sm font-medium">Recovery roadmap</h2><p className="mt-0.5 text-[10px] text-gray-500">Important milestones stay visible while the controls below make every wave directly selectable.</p></div><HealthLegend /></div><RecoveryRoadmapLens waves={waves} selectedWave={safeSelectedWave} setSelectedWave={setSelectedWave} scheduledStrategies={scheduledStrategies} /><div className="mt-4"><WaveNavigator waves={waves} selectedWave={safeSelectedWave} setSelectedWave={setSelectedWave} /></div></section>

    <section className="mt-4 rounded-2xl border border-blue-200 bg-blue-50/35 p-4"><div className="flex items-center justify-between"><div><div className="text-[9px] uppercase tracking-[0.14em] font-semibold text-blue-700">Selected wave workspace</div><h2 className="mt-1 text-base font-medium">Wave {wave.number} · {wave.month}</h2><p className="mt-1 text-[10px] text-gray-600">These details always follow the wave selected above.</p></div><div className="flex items-center gap-5"><div className="text-right"><span className="block text-[9px] text-gray-500">Projected health</span><strong className="text-xl">{wave.before} → <i className={`not-italic ${pulseScoreTone(wave.after).text}`}>{wave.after}</i></strong></div><div className="text-right"><span className="block text-[9px] text-gray-500">Newly aligned</span><strong className="text-xl text-green-700">+{wave.newlyAligned.toLocaleString()}</strong></div><span className={`rounded-full border px-2.5 py-1 text-[9px] ${waveLocked ? 'border-blue-200 bg-blue-50 text-blue-700' : 'border-amber-200 bg-amber-50 text-amber-800'}`}>{waveLocked ? 'Locked · executing' : safeSelectedWave === 0 ? 'Needs review' : 'Draft · refreshes before lock'}</span></div></div></section>

    <div className="mt-3 grid gap-4" style={{gridTemplateColumns:'minmax(0,1.5fr) 300px'}}><WaveSkuPreview wave={wave} waveIndex={safeSelectedWave} onNavigate={onNavigate} /><aside className="rounded-xl border border-gray-200 bg-white p-4"><div className="text-[9px] uppercase tracking-[0.14em] text-gray-500">Wave decision</div><div className="mt-3 grid grid-cols-2 gap-2"><div className="rounded-xl border border-red-100 bg-red-50 p-3"><span className="text-[8px] uppercase tracking-wider text-red-700">DC Returns</span><strong className="mt-1 block text-sm">${wave.returnBudget.toLocaleString()}</strong><small className="text-[8px] text-red-800">{wave.returnUnits.toLocaleString()} SKUs</small></div><div className="rounded-xl border border-green-100 bg-green-50 p-3"><span className="text-[8px] uppercase tracking-wider text-green-700">Store Orders</span><strong className="mt-1 block text-sm">{wave.orderUnits.toLocaleString()}</strong><small className="text-[8px] text-green-800">Outside return budget</small></div></div><dl className="mt-4 space-y-2 text-[10px]"><div className="flex justify-between"><dt className="text-gray-500">Return value remaining</dt><dd className="font-semibold">${wave.remainingReturnValue.toLocaleString()}</dd></div><div className="flex justify-between"><dt className="text-gray-500">Strategy-affected SKUs</dt><dd className="font-semibold">{wave.strategySkus.toLocaleString()}</dd></div></dl><div className={`mt-4 rounded-xl border p-3 ${scheduledStrategy ? 'border-blue-200 bg-blue-50' : 'border-gray-200 bg-gray-50'}`}><div className="flex items-center gap-2 text-[10px] font-medium"><CalendarDays size={12} className={scheduledStrategy ? 'text-blue-700' : 'text-gray-500'} />Strategy before this wave</div>{scheduledStrategy ? <><strong className="mt-2 block text-[10px] text-blue-950">{scheduledStrategy.title}</strong><p className="mt-1 text-[9px] leading-4 text-blue-800">Applied before {wave.month} recommendations refresh.</p></> : <p className="mt-2 text-[9px] leading-4 text-gray-500">Current strategy continues unless a change is scheduled.</p>}{!waveLocked && <button onClick={() => setStrategyEditorOpen(!strategyEditorOpen)} className="mt-2 text-[9px] font-medium text-blue-700">{scheduledStrategy ? 'Change scheduled strategy' : 'Schedule strategy change'}</button>}</div>{strategyEditorOpen && !waveLocked && <div className="mt-3 rounded-xl border border-blue-200 bg-white p-3"><select value={scenarioChoice} onChange={event => setScenarioChoice(event.target.value)} className="w-full rounded-lg border border-gray-200 px-2 py-2 text-[9px]"><option>Expand Spark Plugs assortment</option><option>Reduce premium Brake Pad brands</option><option>Expand seasonal Wipers</option></select><button onClick={scheduleStrategy} className="mt-2 w-full rounded-lg bg-blue-700 px-2 py-2 text-[9px] font-medium text-white">Schedule for {wave.month.split(' ')[0]}</button></div>}<button onClick={() => {if (safeSelectedWave === 0) setLaunched(true);}} className={`mt-4 w-full rounded-xl px-3 py-3 text-[10px] font-medium flex items-center justify-center gap-2 ${waveLocked ? 'border border-blue-200 bg-blue-50 text-blue-700' : 'bg-blue-700 text-white'}`}>{waveLocked ? <><CheckCircle2 size={12} />Wave locked · executing</> : safeSelectedWave === 0 ? <><Lock size={12} />Review & lock Wave 1</> : <><Eye size={12} />Review Wave {wave.number}</>}</button><p className="mt-2 text-center text-[8px] leading-4 text-gray-500">Nothing is exported to the DC without review.</p></aside></div>

    <div className="mt-4"><WaveCategoryExplorer wave={wave} waveIndex={safeSelectedWave} onNavigate={onNavigate} /></div>
    <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 flex gap-3"><Clock3 size={15} className="mt-0.5 text-amber-700" /><p className="text-[10px] leading-5 text-amber-900"><strong>Operational guardrail:</strong> locked waves remain stable. Daily ranking changes and effective-dated Scenario Planner rules rebuild only future waves; Orders stay separate from the DC return-value budget.</p></div>
  </div>;
}

function CompactWaveSelector({ waves, selectedWave, setSelectedWave }) {
  const wave = waves[selectedWave];
  return <div className="border-b border-gray-200 bg-gray-50/70 px-4 py-3">
    <div className="flex items-center gap-2">
      <button aria-label="Previous wave" disabled={selectedWave === 0} onClick={() => setSelectedWave(selectedWave - 1)} className="w-8 h-8 rounded-lg border border-gray-200 bg-white flex items-center justify-center text-gray-600 disabled:opacity-30"><ArrowLeft size={13} /></button>
      <label className="flex items-center gap-2 text-[9px] text-gray-500">Selected wave<select aria-label="Select execution wave" value={selectedWave} onChange={event => setSelectedWave(Number(event.target.value))} className="rounded-lg border border-blue-200 bg-white px-3 py-2 text-[10px] font-semibold text-blue-900 outline-none">{waves.map((item,index) => <option key={item.number} value={index}>Wave {item.number} · {item.month} · health {item.after}</option>)}</select></label>
      <button aria-label="Next wave" disabled={selectedWave === waves.length - 1} onClick={() => setSelectedWave(selectedWave + 1)} className="w-8 h-8 rounded-lg border border-gray-200 bg-white flex items-center justify-center text-gray-600 disabled:opacity-30"><ArrowRight size={13} /></button>
      <div className="ml-2 grid flex-1 gap-1" style={{gridTemplateColumns:`repeat(${waves.length},minmax(24px,1fr))`}}>{waves.map((item,index) => <button key={item.number} onClick={() => setSelectedWave(index)} aria-label={`Select Wave ${item.number}, ${item.month}`} title={`${item.month} · health ${item.after}`} className={`h-8 rounded-md border text-[8px] font-semibold transition ${selectedWave === index ? 'border-blue-700 bg-blue-700 text-white' : 'border-gray-200 bg-white text-gray-500 hover:border-blue-300'}`}>W{item.number}</button>)}</div>
      <div className="ml-2 min-w-[115px] text-right"><strong className="block text-[10px]">{wave.month}</strong><span className="text-[8px] text-gray-500">Health {wave.before} → {wave.after}</span></div>
    </div>
  </div>;
}

function CompactRoadmapLens({ waves, selectedWave, setSelectedWave, scheduledStrategies, onNavigate }) {
  const wave = waves[selectedWave];
  const firstAtRisk = waves.findIndex(item => item.after > 50);
  const firstExcellent = waves.findIndex(item => item.after > 70);
  const points = [{number:0,after:29,month:'Today'},...waves];
  const paths = EXECUTION_CATEGORY_PATHS.slice(0,3).map((item,index) => ({...item,after:Math.min(96,item.before + item.lift + Math.round(selectedWave * (2.8 - index * .18)))}));
  const leadPath = EXECUTION_CATEGORY_PATHS[selectedWave % EXECUTION_CATEGORY_PATHS.length];
  const selectedStrategy = scheduledStrategies[selectedWave];
  const crossesThreshold = scoreBand(wave.before) !== scoreBand(wave.after);
  const waveInsight = selectedWave === 0
    ? <>Pulse recommends using <strong>${wave.returnBudget.toLocaleString()}</strong> of DC return budget to move out <strong>{wave.returnUnits.toLocaleString()} lowest-ranked SKUs</strong> and sequence <strong>{wave.orderUnits.toLocaleString()} top-ranked missing products</strong>. The largest early opportunity is <strong>{leadPath.path.join(' › ')}</strong>. This first wave is projected to align <strong>{wave.newlyAligned.toLocaleString()} additional SKUs</strong> and lift store health from <strong>{wave.before} to {wave.after}</strong>.</>
    : <>Wave {wave.number} focuses next on <strong>{leadPath.path.join(' › ')}</strong>. Pulse allocates <strong>${wave.returnBudget.toLocaleString()}</strong> to {wave.returnUnits.toLocaleString()} recommended Returns and sequences {wave.orderUnits.toLocaleString()} Orders, adding <strong>{wave.newlyAligned.toLocaleString()} aligned SKUs</strong> and moving health from <strong>{wave.before} to {wave.after}</strong>{crossesThreshold ? <>—taking the store from <strong>{scoreBand(wave.before)}</strong> to <strong>{scoreBand(wave.after)}</strong></> : null}. {selectedStrategy ? <><strong>{selectedStrategy.title}</strong> is applied before this wave is rebuilt.</> : <>The current approved strategy continues for this wave.</>}</>;
  return <div>
    <div className="rounded-xl border border-blue-200 bg-gradient-to-r from-blue-50 to-white overflow-hidden"><div className="px-3 py-2 border-b border-blue-100 flex items-center justify-between"><div className="flex items-center gap-2"><span className="w-6 h-6 rounded-lg bg-blue-700 text-white flex items-center justify-center"><Sparkles size={12} /></span><span><strong className="block text-[10px] text-blue-950">Pulse AI analysis · Wave {wave.number}</strong><small className="block text-[7px] text-blue-700">Generated from today’s SKU rankings, DC budget, and approved strategy</small></span></div><span className="rounded-full border border-blue-200 bg-white px-2 py-1 text-[7px] font-medium text-blue-700 flex items-center gap-1"><RefreshCw size={8} />Updates with selected wave</span></div><div className="px-3 py-2.5"><p className="text-[9px] leading-[1.15rem] text-blue-950">{waveInsight}</p><div className="mt-2 flex flex-wrap gap-1.5"><span className="rounded-full border border-blue-100 bg-white px-2 py-1 text-[7px] text-blue-800">Health +{wave.after - wave.before}</span><span className="rounded-full border border-blue-100 bg-white px-2 py-1 text-[7px] text-blue-800">+{wave.newlyAligned.toLocaleString()} aligned</span><span className="rounded-full border border-blue-100 bg-white px-2 py-1 text-[7px] text-blue-800">${wave.remainingReturnValue.toLocaleString()} return value remains</span>{selectedStrategy && <span className="rounded-full border border-violet-200 bg-violet-50 px-2 py-1 text-[7px] text-violet-800"><Beaker size={8} className="inline mr-1" />Strategy change applied</span>}</div><p className="mt-2 text-[7px] text-gray-500">Overall outlook: Critical exit in {waves[firstAtRisk]?.month} · Excellent in {waves[firstExcellent]?.month}</p></div></div>
    <div className="mt-3 rounded-xl border border-gray-200 overflow-hidden"><div className="px-3 py-2.5 border-b border-gray-100 flex items-center justify-between"><div><h3 className="text-xs font-medium">Projected health trajectory</h3><p className="text-[8px] text-gray-500">Click a point or use the selector above.</p></div><HealthLegend /></div><div className="relative h-[210px] px-3 pt-2 pb-1"><div className="absolute left-12 right-3 top-2 h-[27%] rounded-t bg-green-50/70 border-b border-green-100" /><div className="absolute left-12 right-3 top-[30%] h-[20%] bg-amber-50/65 border-b border-amber-100" /><div className="absolute left-12 right-3 top-[50%] bottom-6 rounded-b bg-red-50/45" /><span className="absolute left-1 top-4 text-[7px] text-green-700">Excellent</span><span className="absolute left-1 top-[36%] text-[7px] text-amber-700">At risk</span><span className="absolute left-1 top-[70%] text-[7px] text-red-700">Critical</span><div className="relative ml-10 h-full grid gap-1" style={{gridTemplateColumns:`repeat(${points.length},minmax(24px,1fr))`}}>{points.map((item,pointIndex) => { const waveIndex = pointIndex - 1; const isToday = pointIndex === 0; const active = !isToday && selectedWave === waveIndex; const tone = pulseScoreTone(item.after); const strategy = !isToday && scheduledStrategies[waveIndex]; return <button key={`${item.number}-${item.month}`} disabled={isToday} onClick={() => !isToday && setSelectedWave(waveIndex)} className="relative h-full group"><span className="absolute inset-x-0 bottom-5"><span className={`mx-auto block w-[3px] rounded-t ${tone.bg} ${active ? 'opacity-100' : 'opacity-45'}`} style={{height:`${Math.max(28,item.after * 1.42)}px`}} /><span className={`absolute left-1/2 -translate-x-1/2 -top-2 w-3 h-3 rounded-full border-2 border-white ${tone.bg} ${active ? 'ring-2 ring-blue-500 ring-offset-1' : ''}`} /><strong className={`absolute left-1/2 -translate-x-1/2 -top-6 text-[8px] ${active ? 'text-blue-800' : tone.text}`}>{item.after}</strong>{strategy && <span className="absolute left-1/2 -translate-x-1/2 -top-10 w-4 h-4 rounded border border-blue-200 bg-blue-50 text-blue-700 flex items-center justify-center"><CalendarDays size={8} /></span>}</span><span className={`absolute inset-x-0 bottom-0 text-[7px] ${active ? 'font-semibold text-blue-800' : 'text-gray-400'}`}>{isToday ? 'Now' : `W${item.number}`}</span></button>; })}</div></div></div>
    <div className="mt-3 grid grid-cols-2 gap-3"><section className="rounded-xl border border-gray-200 overflow-hidden"><div className="px-3 py-2 bg-gray-50 flex items-center justify-between"><div><strong className="block text-[10px]">SKU snapshot · Wave {wave.number}</strong><span className="text-[8px] text-gray-500">{wave.returnUnits.toLocaleString()} Returns · {wave.orderUnits.toLocaleString()} Orders</span></div><button onClick={() => onNavigate('sku-list')} className="text-[8px] font-medium text-blue-700">View all</button></div><div className="divide-y divide-gray-100">{[WAVE_RETURNS[0],WAVE_ORDERS[0],WAVE_RETURNS[1]].map((item,index) => { const isReturn = index !== 1; return <button key={`${item.part}-${index}`} onClick={() => onNavigate('sku-list')} className="w-full px-3 py-2 text-left flex items-center gap-2 hover:bg-blue-50/30"><span className={`rounded-full border px-1.5 py-0.5 text-[7px] font-semibold ${isReturn ? 'border-red-200 bg-red-50 text-red-700' : 'border-green-200 bg-green-50 text-green-700'}`}>{isReturn ? 'Return' : 'Order'}</span><span className="min-w-0 flex-1"><strong className="block truncate text-[9px]">{item.name}</strong><small className="block truncate text-[7px] text-gray-500">{item.part} · {item.reason}</small></span><strong className="text-[8px] text-gray-500">{item.rank}</strong></button>; })}</div></section><section className="rounded-xl border border-gray-200 overflow-hidden"><div className="px-3 py-2 bg-gray-50 flex items-center justify-between"><div><strong className="block text-[10px]">Top category paths affected</strong><span className="text-[8px] text-gray-500">3 of 1,487 monitored nodes</span></div><button onClick={() => onNavigate('category-l2')} className="text-[8px] font-medium text-blue-700">Explore</button></div><div className="divide-y divide-gray-100">{paths.map(item => <button key={item.path.join('-')} onClick={() => onNavigate('category-l2')} className="w-full px-3 py-2 text-left hover:bg-blue-50/30"><div className="flex items-center justify-between gap-2"><span className="truncate text-[8px] text-gray-600">{item.path.join(' › ')}</span><strong className={`shrink-0 text-[8px] ${pulseScoreTone(item.after).text}`}>{item.before} → {item.after}</strong></div></button>)}</div></section></div>
  </div>;
}

function CompactWaveInspector({ wave, selectedWave, launched, setLaunched, scheduledStrategy, strategyEditorOpen, setStrategyEditorOpen, scenarioChoice, setScenarioChoice, scheduleStrategy }) {
  const locked = selectedWave === 0 && launched;
  return <aside className="border-l border-gray-200 bg-gray-50/45 p-4"><div className="flex items-start justify-between"><div><div className="text-[8px] uppercase tracking-[0.14em] text-gray-500">Selected wave</div><h3 className="mt-1 text-sm font-medium">Wave {wave.number}</h3><p className="text-[9px] text-gray-500">{wave.month}</p></div><span className={`rounded-full border px-2 py-1 text-[8px] ${locked ? 'border-blue-200 bg-blue-50 text-blue-700' : 'border-amber-200 bg-amber-50 text-amber-800'}`}>{locked ? 'Executing' : selectedWave === 0 ? 'Review' : 'Draft'}</span></div><div className="mt-3 rounded-xl border border-gray-200 bg-white p-3"><div className="text-[8px] text-gray-500">Projected health</div><div className="mt-1 flex items-center gap-2"><strong className="text-2xl">{wave.before}</strong><ArrowRight size={13} className="text-gray-400" /><strong className={`text-2xl ${pulseScoreTone(wave.after).text}`}>{wave.after}</strong><span className="ml-auto text-[8px] text-green-700">+{wave.newlyAligned.toLocaleString()} aligned</span></div></div><div className="mt-3 grid grid-cols-2 gap-2"><div className="rounded-lg border border-red-100 bg-red-50 p-2.5"><span className="text-[7px] uppercase text-red-700">Returns</span><strong className="block text-xs mt-1">${wave.returnBudget.toLocaleString()}</strong><small className="text-[7px] text-red-700">{wave.returnUnits.toLocaleString()} SKUs</small></div><div className="rounded-lg border border-green-100 bg-green-50 p-2.5"><span className="text-[7px] uppercase text-green-700">Orders</span><strong className="block text-xs mt-1">{wave.orderUnits.toLocaleString()}</strong><small className="text-[7px] text-green-700">Outside budget</small></div></div><dl className="mt-3 space-y-2 text-[9px]"><div className="flex justify-between"><dt className="text-gray-500">Return value remaining</dt><dd className="font-semibold">${wave.remainingReturnValue.toLocaleString()}</dd></div><div className="flex justify-between"><dt className="text-gray-500">Strategy-affected SKUs</dt><dd className="font-semibold">{wave.strategySkus.toLocaleString()}</dd></div></dl><div className={`mt-3 rounded-xl border p-3 ${scheduledStrategy ? 'border-blue-200 bg-blue-50' : 'border-gray-200 bg-white'}`}><div className="flex items-center gap-2 text-[9px] font-medium"><CalendarDays size={11} className={scheduledStrategy ? 'text-blue-700' : 'text-gray-500'} />Strategy before wave</div>{scheduledStrategy ? <><strong className="mt-2 block text-[9px] text-blue-950">{scheduledStrategy.title}</strong><p className="mt-1 text-[8px] leading-4 text-blue-800">Applied before the recommendations refresh.</p></> : <p className="mt-2 text-[8px] leading-4 text-gray-500">Current strategy continues unless a change is scheduled.</p>}{!locked && <button onClick={() => setStrategyEditorOpen(!strategyEditorOpen)} className="mt-2 text-[8px] font-medium text-blue-700">{scheduledStrategy ? 'Change strategy' : 'Schedule strategy change'}</button>}</div>{strategyEditorOpen && !locked && <div className="mt-2 rounded-xl border border-blue-200 bg-white p-2.5"><select value={scenarioChoice} onChange={event => setScenarioChoice(event.target.value)} className="w-full rounded-lg border border-gray-200 px-2 py-2 text-[8px]"><option>Expand Spark Plugs assortment</option><option>Reduce premium Brake Pad brands</option><option>Expand seasonal Wipers</option></select><button onClick={scheduleStrategy} className="mt-2 w-full rounded-lg bg-blue-700 px-2 py-2 text-[8px] font-medium text-white">Schedule for {wave.month.split(' ')[0]}</button></div>}<div className="mt-3 rounded-lg border border-blue-100 bg-blue-50/70 p-2.5 flex gap-2"><Sparkles size={10} className="mt-0.5 shrink-0 text-blue-700" /><p className="text-[8px] leading-4 text-blue-900">This wave removes {wave.returnUnits.toLocaleString()} low-ranked SKUs within the DC budget and brings in {wave.orderUnits.toLocaleString()} higher-ranked products.</p></div><button onClick={() => {if (selectedWave === 0) setLaunched(true);}} className={`mt-3 w-full rounded-lg px-3 py-2.5 text-[9px] font-medium flex items-center justify-center gap-2 ${locked ? 'border border-blue-200 bg-blue-50 text-blue-700' : 'bg-blue-700 text-white'}`}>{locked ? <><CheckCircle2 size={11} />Wave locked · executing</> : selectedWave === 0 ? <><Lock size={11} />Review & lock Wave 1</> : <><Eye size={11} />Review Wave {wave.number}</>}</button><p className="mt-1.5 text-center text-[7px] text-gray-500">Nothing exports without review.</p></aside>;
}
...
function PulseStoreWorkspaceScreen({ onNavigate }) {
  ...
  const wave = waves[safeSelectedWave];
  const scheduledStrategy = scheduledStrategies[safeSelectedWave];
  const scheduleStrategy = () => { if (safeSelectedWave === 0 && launched) return; setScheduledStrategies(previous => ({...previous,[safeSelectedWave]:{title:scenarioChoice,detail:'Applied before the wave recommendations refresh.',status:'Scheduled'}})); setStrategyEditorOpen(false); };
  if (view === 'overview') return <StoreOverviewExperience view={view} setView={setView} onNavigate={onNavigate} />;
  if (view === 'categories') return <StoreCategoryHealthExperience view={view} setView={setView} onNavigate={onNavigate} />;
  return <div><Breadcrumb items={['Assortment overview','Store health','ATL-050','Store execution']} /><div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2"><h1 className="text-[22px] font-medium">ATL-050 (DOWNTOWN ATLANTA)</h1><span className={`rounded-full border px-2.5 py-1 text-[10px] ${launched ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-amber-50 border-amber-200 text-amber-800'}`}>{launched ? 'Wave 1 in progress' : 'Plan ready for review'}</span></div><p className="mt-1 text-sm text-gray-500">A compact command center for the store’s recovery journey.</p></div><button onClick={() => onNavigate('sku-list')} className="rounded-full border border-blue-700 bg-white px-4 py-2 text-xs text-blue-700 flex items-center gap-2"><List size={14} />Complete SKU list</button></div><div className="mt-4 flex items-center justify-between"><StorePageTabs view={view} setView={setView} /><div className="flex items-center gap-2 text-xs text-gray-500"><RefreshCw size={13} />Recommendations refreshed today · future waves remain dynamic</div></div>
    <section className="mt-4 rounded-2xl border border-gray-200 bg-white overflow-hidden"><div className="grid grid-cols-[180px_repeat(4,1fr)] border-b border-gray-200"><div className="px-4 py-3 bg-blue-950 text-white flex items-center gap-3"><Route size={15} /><div><strong className="block text-[10px]">Recovery plan</strong><span className="text-[8px] text-blue-200">15 monthly waves</span></div></div><div className="px-4 py-3 border-r border-gray-100"><span className="text-[8px] text-gray-500">Health journey</span><strong className="mt-1 block text-base">29 → <i className="not-italic text-green-700">91</i></strong></div><div className="px-4 py-3 border-r border-gray-100"><span className="text-[8px] text-gray-500">Return value pending</span><strong className="mt-1 block text-base">$148,600</strong></div><button onClick={() => {setLens('budget');setBudgetEditing(true);}} className="px-4 py-3 border-r border-gray-100 text-left"><span className="text-[8px] text-gray-500">Monthly DC return budget</span><strong className="mt-1 flex items-center gap-1 text-base">${Number(budget).toLocaleString()} <PencilLine size={9} className="text-blue-700" /></strong></button><div className="px-4 py-3"><span className="text-[8px] text-gray-500">Projected completion</span><strong className="mt-1 block text-base">{waves[waves.length - 1].month}</strong></div></div><CompactWaveSelector waves={waves} selectedWave={safeSelectedWave} setSelectedWave={setSelectedWave} /><div className="grid items-start" style={{gridTemplateColumns:'155px minmax(0,1fr) 275px'}}><ExecutionLensRail lens={lens} setLens={setLens} /><main className="p-4">{lens === 'roadmap' && <CompactRoadmapLens waves={waves} selectedWave={safeSelectedWave} setSelectedWave={setSelectedWave} scheduledStrategies={scheduledStrategies} onNavigate={onNavigate} />}{lens === 'skus' && <WaveSkuPreview wave={wave} waveIndex={safeSelectedWave} onNavigate={onNavigate} />}{lens === 'categories' && <WaveCategoryExplorer wave={wave} waveIndex={safeSelectedWave} onNavigate={onNavigate} />}{lens === 'budget' && <BudgetUtilizationLens waves={waves} budget={budget} selectedWave={safeSelectedWave} setSelectedWave={setSelectedWave} budgetEditing={budgetEditing} setBudgetEditing={setBudgetEditing} setBudget={setBudget} />}</main><CompactWaveInspector wave={wave} selectedWave={safeSelectedWave} launched={launched} setLaunched={setLaunched} scheduledStrategy={scheduledStrategy} strategyEditorOpen={strategyEditorOpen} setStrategyEditorOpen={setStrategyEditorOpen} scenarioChoice={scenarioChoice} setScenarioChoice={setScenarioChoice} scheduleStrategy={scheduleStrategy} /></div></section>
    <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 flex gap-3"><Clock3 size={14} className="mt-0.5 text-amber-700" /><p className="text-[9px] leading-4 text-amber-900"><strong>Operational guardrail:</strong> locked waves remain stable. Daily ranking changes and Scenario Planner rules rebuild only future waves; Orders stay separate from the DC return-value budget.</p></div></div>;
}
...
function PulseGetWellEnablementScreen({ onNavigate }) {
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between"><div><h1 className="text-[22px] font-medium">Execution enablement</h1><p className="text-sm text-gray-500 mt-1">Turn on a digital worker after the DC assigns a monthly return-value budget. Review and edit each store execution journey inside its store page.</p></div><button onClick={() => setEnabled(new Set(ENABLEMENT_STORES.map(store => store.id)))} className="rounded-full bg-blue-700 hover:bg-blue-800 text-white px-4 py-2.5 text-xs font-medium flex items-center gap-2"><Sparkles size={14} />Enable all ready stores</button></div>
      <div className="grid grid-cols-4 gap-3 mt-5"><div className="rounded-2xl border border-gray-200 bg-white p-4"><span className="text-xs text-gray-500">Digital workers on</span><strong className="text-2xl block mt-2">{enabled.size}</strong><small className="text-[11px] text-gray-500">of 195 stores</small></div><div className="rounded-2xl border border-gray-200 bg-white p-4"><span className="text-xs text-gray-500">Monthly DC return budget</span><strong className="text-2xl block mt-2">$62,000</strong><small className="text-[11px] text-gray-500">assigned across enabled stores</small></div><div className="rounded-2xl border border-gray-200 bg-white p-4"><span className="text-xs text-gray-500">Plans needing review</span><strong className="text-2xl block mt-2">2</strong><small className="text-[11px] text-gray-500">before launch</small></div><div className="rounded-2xl border border-gray-200 bg-white p-4"><span className="text-xs text-gray-500">Return value queued</span><strong className="text-2xl block mt-2">$411.6K</strong><small className="text-[11px] text-gray-500">across enabled stores</small></div></div>
...
function BudgetExecutionEnablementScreen({ onNavigate }) {
  const [enabled, setEnabled] = useState(() => new Set(ENABLEMENT_STORES.slice(0,5).map(store => store.id)));
  const [selectedId, setSelectedId] = useState('ATL-050');
  const [filter, setFilter] = useState('All stores');
  const selected = ENABLEMENT_STORES.find(store => store.id === selectedId) || ENABLEMENT_STORES[0];
  const visible = ENABLEMENT_STORES.filter(store => filter === 'All stores' || (filter === 'Enabled' ? enabled.has(store.id) : !enabled.has(store.id)));
  const toggle = id => setEnabled(previous => { const next = new Set(previous); next.has(id) ? next.delete(id) : next.add(id); return next; });
  const enabledStores = ENABLEMENT_STORES.filter(store => enabled.has(store.id));
  const assignedBudget = enabledStores.reduce((sum, store) => sum + store.capacity, 0);
  const queuedValue = enabledStores.reduce((sum, store) => sum + store.actions, 0);

  return <div>
    <Breadcrumb items={['Pulse AI','Execution enablement']} />
    <div className="flex items-start justify-between"><div><h1 className="text-[22px] font-medium">Execution enablement</h1><p className="text-sm text-gray-500 mt-1">Enable a digital worker after the DC assigns a monthly return-value budget to the store.</p></div><button onClick={() => setEnabled(new Set(ENABLEMENT_STORES.map(store => store.id)))} className="rounded-full bg-blue-700 hover:bg-blue-800 text-white px-4 py-2.5 text-xs font-medium flex items-center gap-2"><Sparkles size={14} />Enable all ready stores</button></div>
    <div className="grid grid-cols-4 gap-3 mt-5"><div className="rounded-2xl border border-gray-200 bg-white p-4"><span className="text-xs text-gray-500">Digital workers on</span><strong className="text-2xl block mt-2">{enabled.size}</strong><small className="text-[11px] text-gray-500">of 195 stores</small></div><div className="rounded-2xl border border-gray-200 bg-white p-4"><span className="text-xs text-gray-500">Monthly DC return budget</span><strong className="text-2xl block mt-2">${assignedBudget.toLocaleString()}</strong><small className="text-[11px] text-gray-500">assigned across enabled stores</small></div><div className="rounded-2xl border border-gray-200 bg-white p-4"><span className="text-xs text-gray-500">Plans needing review</span><strong className="text-2xl block mt-2">2</strong><small className="text-[11px] text-gray-500">before launch</small></div><div className="rounded-2xl border border-gray-200 bg-white p-4"><span className="text-xs text-gray-500">Return value queued</span><strong className="text-2xl block mt-2">${(queuedValue/1000).toFixed(1)}K</strong><small className="text-[11px] text-gray-500">future DC P&amp;L impact</small></div></div>
    <div className="mt-4 grid grid-cols-[1fr_300px] gap-4">
      <section className="rounded-2xl border border-gray-200 bg-white overflow-hidden"><div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between"><div className="inline-flex rounded-full border border-gray-200 bg-gray-50 p-1">{['All stores','Enabled','Not enabled'].map(item => <button key={item} onClick={() => setFilter(item)} className={`rounded-full px-4 py-2 text-xs ${filter === item ? 'bg-gray-950 text-white' : 'text-gray-600'}`}>{item}</button>)}</div><span className="text-xs text-gray-500">{visible.length} stores · lowest health first</span></div><div className="grid px-5 py-3 bg-gray-50 text-[10px] uppercase tracking-wider text-gray-500" style={{gridTemplateColumns:'1.35fr 1fr 120px 80px'}}><span>Store</span><span>Projected recovery</span><span>Return budget</span><span>Worker</span></div><div className="divide-y divide-gray-100">{visible.map(store => { const isOn = enabled.has(store.id); return <button key={store.id} onClick={() => setSelectedId(store.id)} className={`w-full grid items-center px-5 py-3 text-left hover:bg-gray-50 ${selected.id === store.id ? 'bg-blue-50/50' : ''}`} style={{gridTemplateColumns:'1.35fr 1fr 120px 80px'}}><span className="flex items-center gap-3"><i className={`not-italic w-9 h-9 rounded-xl flex items-center justify-center ${isOn ? 'bg-blue-50 text-blue-700' : 'bg-gray-100 text-gray-400'}`}><Bot size={16} /></i><span><strong className="text-xs block">{store.id} · {store.name}</strong><small className="text-[10px] text-gray-500">{isOn ? store.status : 'Not enabled'}</small></span></span><span className="pr-5"><span className="flex justify-between text-[10px]"><b>{store.score}</b><b className="text-green-700">{store.target}</b></span><span className="block mt-1 h-1.5 bg-gray-100 rounded-full overflow-hidden"><i className="block h-full bg-green-600 rounded-full" style={{width:`${store.target}%`}} /></span><small className="text-[9px] text-gray-500">{store.waves} monthly waves</small></span><strong className="text-xs">${store.capacity.toLocaleString()}<small className="font-normal text-gray-500"> / mo</small></strong><span onClick={event => event.stopPropagation()}><button aria-label={`${isOn ? 'Disable' : 'Enable'} ${store.id}`} onClick={() => toggle(store.id)} className={`w-11 h-6 rounded-full p-0.5 transition-colors ${isOn ? 'bg-blue-700' : 'bg-gray-300'}`}><i className={`not-italic block w-5 h-5 rounded-full bg-white transition-transform ${isOn ? 'translate-x-5' : ''}`} /></button></span></button>; })}</div></section>
      <aside className="rounded-2xl border border-gray-200 bg-white p-5"><div className="text-[10px] uppercase tracking-wider text-gray-500">Selected store</div><h2 className="font-medium mt-1">{selected.id}</h2><p className="text-xs text-gray-500">{selected.name}</p><div className="mt-4 rounded-xl bg-gray-50 p-4"><div className="text-xs text-gray-500">Projected health</div><div className="mt-2 flex items-center gap-2"><strong className="text-3xl">{selected.score}</strong><ArrowRight size={15} className="text-gray-400" /><strong className="text-3xl text-green-700">{selected.target}</strong></div></div><dl className="mt-4 space-y-3 text-xs"><div className="flex justify-between"><dt className="text-gray-500">Return-value backlog</dt><dd className="font-semibold">${selected.actions.toLocaleString()}</dd></div><div className="flex justify-between"><dt className="text-gray-500">Monthly return budget</dt><dd className="font-semibold">${selected.capacity.toLocaleString()}</dd></div><div className="flex justify-between"><dt className="text-gray-500">Estimated waves</dt><dd className="font-semibold">{selected.waves}</dd></div></dl><div className="mt-5 rounded-xl border border-blue-100 bg-blue-50 p-3"><div className="text-xs font-medium text-blue-900">Digital worker inputs</div><div className="mt-2 space-y-2 text-[10px] text-blue-800"><div className="flex gap-2"><CheckCircle2 size={12} />Latest SKU ranking and item value</div><div className="flex gap-2"><CheckCircle2 size={12} />Scenario Planner strategy</div><div className="flex gap-2"><CheckCircle2 size={12} />DC monthly return budget</div></div></div><button disabled={!enabled.has(selected.id)} onClick={() => onNavigate('store-detail')} className="mt-4 w-full rounded-lg bg-blue-700 disabled:bg-gray-300 text-white px-4 py-3 text-xs font-medium flex items-center justify-center gap-2">Open store execution <ArrowRight size={13} /></button><p className="text-[10px] text-center text-gray-500 mt-2">SKU quantity varies because item values vary.</p></aside>
    </div>
  </div>;
}
...
function HowItWorksScreen() {
  const features = [
    ...
    ['Store execution · MVP','Fill each monthly wave with the worst-ranked Returns up to the DC dollar budget, sequence the best missing products to Order, project health improvement, lock active work, and refresh only future waves.'],
  ];
  ...
  return (
    <div className="pulse-shell flex min-h-screen bg-gray-50 text-gray-900" style={{ fontFamily: 'Inter, ui-sans-serif, system-ui, -apple-system, sans-serif' }}>
      ...
      <main className="pulse-main flex-1 overflow-auto px-7 py-6">
        ...
        {screen === 'get-well' && <BudgetExecutionEnablementScreen onNavigate={setScreen} />}
